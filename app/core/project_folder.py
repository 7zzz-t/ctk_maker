...
def inspect_picked_folder(folder: str | Path | None) -> PickedFolderResult:
    ...
    if not folder:
        return PickedFolderResult(
            "none",
            message=tr("project_folder.no_folder", "No folder selected."),
        )
    p = Path(folder)
    if not p.is_dir():
        return PickedFolderResult(
            ...
            folder=p,
            message=tr(
                "project_folder.not_a_folder", "Not a folder:\n{p}",
            ).format(p=p),
        )
    if (p / PROJECT_META_FILE).is_file():
    ...
    return PickedFolderResult(
        ...
        folder=p,
        message=tr(
            "project_folder.not_a_project_detail",
