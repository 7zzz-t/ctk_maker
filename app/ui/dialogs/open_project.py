...
class _AmbiguousProjectPicker(DarkDialog):
    ...
    def __init__(self, parent, folder: Path, candidates: list[Path]) -> None:
        ...
        self.title(tr("open_project.pick_title", "Pick project file"))
        self.result: Path | None = None
        self._candidates = candidates
        s = dialog_scaling(self)

        hero_header(self, tr("open_project.pick_title", "Pick project file"), "info", s)
        tk.Label(
            self,
            text=tr(
                "open_project.several_found",
                "Several '.ctkproj' files were found in:\n{folder}\n\n"
        ...
        flat_button(
            btn_row, tr("open_project.open", "Open"), "accent", self._on_ok, s,
        ).pack(side="right")
        flat_button(
            btn_row, tr("open_project.cancel", "Cancel"), "ghost", self._on_cancel, s,
...
def prompt_open_project_folder(
    ...
    folder = filedialog.askdirectory(
        parent=parent,
        title=tr(
            "open_project.folder_picker_title",
            "Open project (pick the project folder)",
        ),
    ...
    if result.kind == "multi_page":
        ...
        try:
            ...
            show_error(
                tr("open_project.open_failed", "Open failed"),
                tr(
                    "open_project.meta_read_error",
        ...
        if entry is None or not entry.get("file"):
            show_error(
                tr("open_project.open_failed", "Open failed"),
                tr("open_project.no_active_page", "project.json has no active page."),
                parent=parent,
            )
            return None
