...
class AboutDialog(DarkDialog):
    def __init__(self, parent, app_version: str = ""):
        ...
        self.title(tr("about.title", "About CTkMaker"))
    ...
    def _build(self, version: str) -> None:
        ...
        tk.Label(
            self, text=tr("about.app_name", "CTkMaker"),
        ...
        tk.Label(
            ...
            text=tr(
                "about.tagline",
                "Design CustomTkinter, visually — for free.",
            ),
            bg=PANEL, fg=_ABT_DIM, font=ui_font(round(10 * s)),
            justify="center",
        ...
        tk.Label(
            self, text=tr("about.built_with", "Built with"),
        ...
        for name, url, lic in _BUILT_WITH:
            ...
            tk.Label(
                row, text=f"{tr('about.lib_' + name.lower().replace(' ', '_'), name)}  ", bg=PANEL, fg=_ABT_FG,
            ...
            tk.Label(
                row, text=f"  ({tr('about.license_' + lic.lower().replace('.', '_'), lic)})", bg=PANEL, fg=_ABT_DIM,
        ...
        tk.Label(
            self, text=tr("about.links", "Links"),
            bg=PANEL, fg=_ABT_FG, font=ui_font(round(10 * s), "bold"),
        ).pack(**pad, pady=(0, round(6 * s)))
        for name, url in _PROJECT_LINKS:
            ...
            row.pack(fill="x", padx=round(24 * s), pady=1)
            tk.Label(
                row, text=f"{tr('about.link_' + name.lower().replace(' ', '_'), name)}  ", bg=PANEL, fg=_ABT_FG,
                font=ui_font(round(10 * s)), anchor="w",
            ).pack(side="left")
            link = tk.Label(
                row, text=url, bg=PANEL, fg=_ABT_LINK,
        ...
        bmc_btn = tk.Button(
            bmc_row,
            text=tr("about.buy_me_a_coffee", "Buy me a coffee"),
        ...
        flat_button(self, tr("about.close", "Close"), "ghost", self.destroy, s).pack(
