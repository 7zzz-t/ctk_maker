...
class RenameDialog(ManagedToplevel):
    ...
    def __init__(
        ...
        self.window_title = tr("rename.title", "Rename Widget")
        self._initial = initial_value
        self._label = label if label is not None else tr(
    ...
    def build_content(self) -> ctk.CTkFrame:
        ...
        style.secondary_button(
            footer, tr("rename.cancel", "Cancel"), command=self._on_cancel,
        ).grid(row=0, column=0, sticky="ew", padx=(0, 4))
        style.primary_button(
            footer, tr("rename.ok", "OK"), command=self._on_ok,
