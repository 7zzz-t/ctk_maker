"""Quick modal for Form → Add Dialog flow. Asks for name + size and
returns ``(name, w, h)`` on OK, ``None`` on Cancel.
"""
...
_PRESET_KEYS: dict[str, str] = {
    "Same as Main": "add_dialog.preset_same_as_main",
    "Alert — 380 × 160": "add_dialog.preset_alert",
    "Compact — 420 × 280": "add_dialog.preset_compact",
    "Medium — 520 × 380": "add_dialog.preset_medium",
    "Settings — 640 × 480": "add_dialog.preset_settings",
    "Wizard — 720 × 520": "add_dialog.preset_wizard",
    "Custom": "add_dialog.preset_custom",
}


def _preset_label(name: str) -> str:
    return tr(_PRESET_KEYS[name], name)
...
class AddDialogSizeDialog(ManagedToplevel):
    ...
    def __init__(
        ...
        self.window_title = tr("add_dialog.title", "Add dialog")
        ...
        self._preset_var = tk.StringVar(
            master=parent, value=_preset_label(DIALOG_PRESETS[0][0]),
        )
    ...
    def build_content(self) -> ctk.CTkFrame:
        ...
        style.styled_label(body, tr("add_dialog.name", "Name")).grid(
        ...
        style.styled_label(body, tr("add_dialog.size_preset", "Size Preset")).grid(
            row=2, column=0, columnspan=2, sticky="w", pady=(0, 4),
        )
        ctk.CTkOptionMenu(
            body,
            values=[_preset_label(label) for label, _ in DIALOG_PRESETS],
        ...
        style.styled_label(body, tr("add_dialog.width", "Width")).grid(
            row=4, column=0, sticky="w", pady=(0, 4),
        )
        style.styled_label(body, tr("add_dialog.height", "Height")).grid(
        ...
        style.secondary_button(
            footer, tr("add_dialog.cancel", "Cancel"), command=self._on_cancel,
        ).grid(row=0, column=0, sticky="ew", padx=(0, 4))
        style.primary_button(
            footer, tr("add_dialog.add", "Add"), command=self._on_ok,
        ).grid(row=0, column=1, sticky="ew", padx=(4, 0))
        return container

    def _on_preset_change(self, label: str) -> None:
        for name, size in DIALOG_PRESETS:
            if _preset_label(name) != label:
                continue
    ...
    def _on_size_edited(self, *_args) -> None:
        ...
        if self._suspend_preset_sync:
            return
        custom_label = _preset_label("Custom")
        if self._preset_var.get() != custom_label:
            self._preset_var.set(custom_label)
    ...
    def _on_ok(self) -> None:
        ...
        try:
            ...
            self.bell()
            show_warning(
                tr("add_dialog.invalid_size_title", "Invalid size"),
                tr(
                    "add_dialog.invalid_size_msg",
                    "Width and height must be whole numbers "
                    "between {min} and {max}.",
                ).format(min=self._SIZE_MIN, max=self._SIZE_MAX),
                parent=self,
            )
            return
        if not (
            ...
            self.bell()
            show_warning(
                tr("add_dialog.out_of_range_title", "Size out of range"),
                tr(
                    "add_dialog.out_of_range_msg",
                    "Dialog width and height must be between "
                    "{min} and {max} pixels.",
                ).format(min=self._SIZE_MIN, max=self._SIZE_MAX),
                parent=self,
            )
