...
class RenamePageDialog(DarkDialog):
    ...
    def __init__(self, parent, current_name: str) -> None:
        ...
        self.title(tr("rename_page.title", "Rename page"))
    ...
    def _build(self) -> None:
        ...
        hero_header(self, tr("rename_page.title", "Rename page"), "info", s)

        tk.Label(
            self,
            text=tr(
                "rename_page.current_name", "Current name: {name}",
            ).format(name=self._current),
            bg=PANEL, fg=FG_DIM, font=f_dim,
        ).pack(**pad, anchor="w", pady=(round(4 * s), round(8 * s)))

        tk.Label(
            self, text=tr("rename_page.new_name", "New name:"),
        ...
        tk.Label(
            self, text=tr("rename_page.affects", "⚠ This rename will affect:"),
        ...
        self._ok_btn = flat_button(
            btn_row, tr("rename_page.rename", "Rename"), "accent", self._on_ok, s,
        )
        self._ok_btn.configure(disabledforeground="#888888")
        self._ok_btn.pack(side="right")
        flat_button(
            btn_row, tr("rename_page.cancel", "Cancel"), "ghost", self._on_cancel, s,
    ...
    def _refresh_preview(self) -> None:
        ...
        if not new_slug or new_slug == self._current_slug:
            ...
            self._preview_label.configure(
                text=tr(
                    "rename_page.placeholder",
                    "  (enter a different name to see the changes)",
                ),
                fg=FG_DIM,
            )
        ...
        bullets = tr(
            "rename_page.preview_bullet",
            "  • {old}.ctkproj → {new}.ctkproj",
        ).format(old=old, new=new_slug)
