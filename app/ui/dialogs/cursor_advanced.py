...
class CursorAdvancedDialog(DarkDialog):
    def __init__(self, parent) -> None:
        ...
        self.title(tr("cursor.title", "Advanced Cursor"))
    ...
    def _build(self) -> None:
        s = self._s
        hero_header(self, tr("cursor.title", "Advanced Cursor"), "info", s)
        tk.Label(
            self,
            text=tr(
                "cursor.description",
                "OS-specific cursors render only on the matching system. "
        ...
        for os_key in ("Windows", "Darwin", "Linux"):
            label_text = tr(
                "cursor.os_" + os_key.lower(), _OS_LABELS[os_key],
            )
        ...
        flat_button(
            btn_row, tr("cursor.cancel", "Cancel"), "ghost", self._on_cancel, s,
        ).pack(side="right")
