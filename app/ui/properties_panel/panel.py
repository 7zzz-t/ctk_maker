...
class PropertiesPanel(CommitMixin, SchemaMixin, ctk.CTkFrame):
    ...
    def _build_chrome(self) -> None:
        ...
        tk.Label(
            self._name_row, text=tr("props.panel.name", "Name"),
            bg=BG, fg=STATIC_FG,
        ...
        self._edit_btn = ctk.CTkButton(
            self._name_row, text="" if edit_icon else tr("prop.edit", "Edit"),
            image=edit_icon,
        ...
        tk.Label(
            self._desc_row, text=tr("props.panel.desc", "Desc"),
            bg=BG, fg=STATIC_FG,
            font=ui_font(10), anchor="w",
        ).pack(side="left", padx=(6, 8))
        ...
        self._desc_preview_var = tk.StringVar(
            value=tr(
                "prop.click_to_add_description", "Click to add description…",
            ),
        )
        ...
        self._desc_edit_btn = ctk.CTkButton(
            self._desc_row, text="" if desc_edit_icon else tr("prop.edit", "Edit"),
    ...
    def _build_tree(self) -> None:
        ...
        tk.Label(
            header_bar, text=tr("prop.property", "Property"), bg=TREE_HEADING_BG,
            fg=TREE_HEADING_FG, font=ui_font(10, "bold"),
        ).grid(row=0, column=0, sticky="nsew")
        tk.Label(
            header_bar, text=tr("prop.value", "Value"), bg=TREE_HEADING_BG,
            fg=TREE_HEADING_FG, font=ui_font(10, "bold"),
    ...
    def _open_description_dialog(self) -> None:
        ...
        dialog = TextEditorDialog(
            self.winfo_toplevel(),
            tr("prop.description_title", "Description: {label}").format(label=label),
            before,
    ...
    def _update_description_preview(self, node) -> None:
        ...
        if not desc:
            preview = tr("prop.click_to_add_description", "Click to add description…")
            fg = "#666666"
        else:
    ...
    def _show_event_menu(self, event, iid: str) -> None:
        ...
        if kind == "pending":
            ...
            self._add_menu_item(
                menu, tr("prop.move_down", "Move down"),
                lambda: self._reorder_event_method(
                    self.current_id, event_key,
                    method_index, method_index + 1,
                ),
            ...
            menu.add_separator()
            menu.add_command(
                label=tr("prop.delete_action", "Delete action…"),
    ...
    def _open_target_retarget_picker(
        ...
        if not comp_targets:
            ...
            menu.add_command(
                label=tr("prop.no_scripts_attached", "No scripts attached — add one in the Scripts group"),
                command=lambda: None,
            ...
            for _script_rel, cls, scope in comp_targets:
                label_scope = (
                    tr("prop.this_widget", "this widget")
                    if scope == "widget"
                    else tr("prop.window", "window")
                )
    ...
    def _open_function_picker(
        ...
        if not methods:
            ...
            menu.add_command(
                label=(
                    tr("prop.no_public_methods", "No public methods in {cls}").format(cls=cls)
                    if cls
                    else tr("prop.script_not_set", "Script not set")
                ),
                command=lambda: None,
                foreground="#777777", activeforeground="#777777",
                activebackground="#2d2d30",
    ...
    def _open_component_picker(self, node) -> None:
        ...
        menu = tk.Menu(self.tree, tearoff=0, **menu_style())
        menu.add_command(
            label=tr("props.panel.new_script", "✚  New script…"),
    ...
    def _create_and_attach_script(self, node) -> None:
        ...
        if scripts_dir is None:
            show_info(
                tr("props.panel.save_first_title", "Save first"),
                tr(
                    "props.panel.save_first_message",
        ...
        if result is None:
            show_error(
                tr(
                    "props.panel.create_script_error_title",
                    "Couldn't create script",
                ),
                tr(
                    "props.panel.create_script_error_message",
    ...
    def _show_local_var_menu(self, event) -> None:
        ...
        menu.add_command(
            label=tr(
                "props.panel.open_variables_editor", "Open Variables Editor",
            ),
    ...
    def _show_binding_menu(self, event, pname, prop, node) -> None:
        ...
        if bound_entry is not None:
            menu.add_command(
                label=tr("prop.unbind_from", "Unbind from: {name}").format(
        ...
        if not global_vars and not local_vars:
            ...
            if visible_total == 0:
                bind_submenu.add_command(
                    label=tr("prop.no_variables_yet", "(no variables yet)"),
                ...
                bind_submenu.add_command(
                    label=tr("prop.no_type_variables", "(no {type} variables)").format(
                        type=type_hint,
                    ),
                    foreground=muted_label_fg,
                    activeforeground=muted_label_fg,
            ...
            if global_vars:
                bind_submenu.add_command(
                    label=tr("props.panel.global_label", "Global"),
                    foreground=muted_label_fg,
                    activeforeground=muted_label_fg,
                    activebackground="#2d2d30",
                    command=lambda: None,
            ...
            if local_vars:
                ...
                doc_label = (
                    ...
                    else tr("props.panel.local_fallback", "Local")
                )
                bind_submenu.add_command(
                    label=tr(
                        "props.panel.local_scope_label", "Local: {doc}",
                    ).format(doc=doc_label),
                    foreground=muted_label_fg,
                    activeforeground=muted_label_fg,
                    activebackground="#2d2d30",
                    command=lambda: None,
            ...
            bind_submenu.add_separator()
        bind_submenu.add_command(
            label=tr("prop.create_new_global", "+ Create new global variable…"),
            foreground=global_header_fg,
        ...
        bind_submenu.add_command(
            label=tr("prop.create_new_local", "+ Create new local variable…"),
            foreground=local_header_fg,
        ...
        menu.add_cascade(label=tr("prop.bind_to_variable", "Bind to variable"), menu=bind_submenu)
        menu.add_separator()
        menu.add_command(
            label=tr("prop.open_variables_window", "Open Variables window…"),
    ...
    def _bind_property(self, pname: str, prop: dict, var_id: str) -> None:
        ...
        if (
            ...
            ok = ask_ok_cancel(
                tr("prop.bind_non_boolean_int", "Bind to non-boolean int?"),
                (
                    tr(
                        "prop.bind_non_boolean_int_msg",
                        "Variable '{name}' holds {default!r}, "
                        "which isn't a boolean value (0 / 1).\n\n"
                        ...
                        "the current value ({default}) will be lost."
                        "\n\nBind anyway?",
                    ).format(
                        name=entry.name, default=entry.default,
                    )
                ),
                parent=self.winfo_toplevel(),
                ok_text=tr("prop.bind_anyway", "Bind anyway"),
            )
            if not ok:
                return
    ...
    def _create_and_bind(
        ...
        if scope == "local":
            ...
            title = tr(
                "props.panel.create_local_var_bind",
                "Create local variable + bind",
            )
        else:
            existing = {v.name for v in self.project.variables}
            title = tr(
                "props.panel.create_global_var_bind",
                "Create global variable + bind",
            )
