...
# Localized accessors
#
# The dicts above keep the English source text as the tr() default.
# Lookups go through these helpers so translation resolves at hover
# time (not at import) — a runtime language switch needs no restart.
# ---------------------------------------------------------------------
def _help_tr(kind: str, help_id: str, field: str, default: str | None):
    if default is None:
        return None
    safe = help_id.replace(":", "_").replace("/", "_").lower()
    return tr(f"props.help.{kind}.{safe}.{field}", default)


def row_help(iid: str) -> dict[str, str | None] | None:
    """Localized help entry for a collapsible parent row iid
    (``pair:*`` / ``g:*`` / ``events:*``). ``None`` if no help exists."""
    entry = ROW_HELP.get(iid)
    if entry is None:
        return None
    return {
        "description": _help_tr(
            "row", iid, "description", entry.get("description"),
        ),
        "warning": _help_tr("row", iid, "warning", entry.get("warning")),
    }


def property_help(pname: str) -> dict[str, str | None] | None:
    """Localized help entry for a schema property name.
    ``None`` if no help exists."""
    entry = PROPERTY_HELP.get(pname)
    if entry is None:
        return None
    return {
        "description": _help_tr(
            "prop", pname, "description", entry.get("description"),
        ),
        "warning": _help_tr("prop", pname, "warning", entry.get("warning")),
    }
