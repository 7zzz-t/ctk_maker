...
def _anchor_key(code: str) -> str:
    return f"props.anchor.{code}"


def anchor_label(code: str) -> str:
    """Localized, human-readable label for a stored anchor code.

    Unknown codes pass through unchanged so raw values stay legible
    instead of surfacing a bare translation key.
    """
    english = ANCHOR_CODE_TO_LABEL.get(code)
    if english is None:
        return code
    return tr(_anchor_key(code), english)


def anchor_code(label: str) -> str | None:
    """Inverse of :func:`anchor_label` — map the localized menu label
    back to the stored code. ``None`` when no code matches."""
    for code, english in ANCHOR_CODE_TO_LABEL.items():
        if tr(_anchor_key(code), english) == label:
            return code
    return None


def anchor_dropdown_order() -> list[str]:
    """Localized anchor labels in the fixed 3×3 dropdown order."""
    return [anchor_label(code) for code in ANCHOR_CODE_TO_LABEL]
