...
def _binding_chip_text(project, value) -> str | None:
    ...
    return entry.name if entry is not None else tr(
        "props.schema.bound_missing", "(missing)",
    )
...
class SchemaMixin:
    ...
    def _populate_schema(self, descriptor, properties: dict, node=None) -> None:
        ...
        while i < len(schema):
            prop = schema[i]
            group = prop.get(
                "group", tr("props.schema.group_general", "General"),
            )
            ...
            if group != current_group:
                ...
                self.tree.insert(
                    "", "end", iid=group_iid,
                    text=prop_group_label(group), values=("",), open=True,
                    tags=("class",),
                )
                current_group = group
            ...
            if subgroup != current_subgroup:
                if subgroup:
                    ...
                    self.tree.insert(
                        ...
                        text=prop_subgroup_label(subgroup),
    ...
    def _populate_local_variables_group(self) -> None:
        ...
        self.tree.insert(
            "", "end", iid=group_iid,
            text=tr("prop_schema.local_variables", "Local Variables"), values=("",), open=True,
            tags=("class",),
        )
        ...
        if not locals_list:
            self.tree.insert(
                ...
                text="", values=(tr("prop_schema.no_local_variables", "no local variables"),),
                tags=("disabled",),
            )
            return
    ...
    def _populate_script_variables_group(self, node) -> None:
        ...
        for ci, comp in enumerate(comps):
            ...
            self.tree.insert(
                "", "end", iid=group_iid,
                text=tr(
                    "props.schema.script_group", "{cls} (Script)",
                ).format(cls=cls),
                values=("",), open=True,
                tags=("class",),
            )
    ...
    def _render_field_editor(
        ...
        if bound_id is not None:
            name = by_id[bound_id].name if bound_id in by_id else tr("prop_schema.missing", "missing")
    ...
    def _open_variable_picker(
        ...
        if choices:
            for v in choices:
                ...
                scope_lbl = (
                    tr("props.schema.scope_local", "local")
                    if v.scope == "local"
                    else tr("props.schema.scope_global", "global")
                )
            ...
            menu.add_command(
                label=tr("prop_schema.no_matching_variables", "No matching variables"),
                command=lambda: None,
        ...
        if bound_id is not None:
            menu.add_separator()
            menu.add_command(
                label=tr("prop_schema.clear_binding", "Clear binding"),
    ...
    def _populate_node_scripts_group(self, node) -> None:
        ...
        self.tree.insert(
            "", "end", iid=group_iid,
            text=tr("prop_schema.scripts", "Scripts"), values=("",), open=True,
            tags=("class",),
        )
    ...
    def _node_script_add_label(self, row_iid: str, on_click) -> None:
        ...
        chip = tk.Label(
            self.tree,
            text=tr("prop_schema.add_script", "Add Script"),
            bg="#262626", fg="#999999",
            font=ui_font(11), anchor="w", padx=4, pady=0,
    ...
    def _node_script_open_label(self, row_iid: str, script_rel: str) -> None:
        ...
        prefix = tr("props.schema.edit_prefix", "Edit [")
    ...
    def _populate_events_group(self, node) -> None:
        ...
        self.tree.insert(
            "", "end", iid=group_iid,
            text=tr("props.schema.events", "Events"), values=("",), open=True,
            tags=("class",),
        )
        ...
        def _ensure_advanced_group() -> str:
            ...
            if not advanced_inserted:
                self.tree.insert(
                    ...
                    text=tr("prop_schema.advanced", "Advanced"), values=("",),
    ...
    def _render_event_row(
        ...
        if methods:
            preview = tr(
                "prop_schema.n_actions",
                "({n} action{s})",
            ).format(
                n=len(methods),
                s=("s" if len(methods) != 1 else ""),
            )
        else:
            preview = tr("prop_schema.no_action", "no action")
    ...
    def _open_handler_function_menu(
        ...
        if has_method:
            ...
            menu.add_command(
                label=tr("prop_schema.delete", "Delete"), command=lambda: None,
    ...
    def _target_label_for_entry(
        ...
        * ``"Target:"`` — pending placeholder (no target picked
        ...
        if isinstance(handler_entry, dict) and (
            ...
            if not cls:
                return tr("prop_schema.target", "Target:"), tr("prop_schema.add_target", "Add target"), None, False
            scope = handler_entry.get("scope")
            if not _script_call_resolvable(self.project, node, cls, scope):
                return tr("prop_schema.script", "Script:"), cls, tr("prop_schema.script_unavailable", "script unavailable"), True
            return tr("prop_schema.script", "Script:"), cls, None, True
    ...
    def _method_and_params_for_entry(
        ...
        if isinstance(handler_entry, dict) and (
            ...
            if not method:
                return tr("prop_schema.pick_function", "Pick function…"), None, []
            return method, None, []
        return tr("prop_schema.pick_function", "Pick function…"), None, []
    ...
    def _attach_pending_target_box(
        ...
        if self.overlays is None:
            return
        lbl = tk.Label(
            self.tree, text=tr("prop_schema.add_target_dots", "Add target…"), bg=VALUE_BG, fg="#999999",
            font=ui_font(11), anchor="w", padx=4, pady=0,
            borderwidth=0, highlightthickness=0, cursor="hand2",
        )
        lbl._full_text = tr("prop_schema.add_target_dots", "Add target…")
    ...
    def _attach_disabled_function_box(self, row_iid: str) -> None:
        ...
        if self.overlays is None:
            return
        lbl = tk.Label(
            self.tree, text=tr("prop_schema.pick_function", "Pick function…"), bg="#262626", fg="#5a5a5a",
            font=ui_font(11), anchor="w", padx=4, pady=0,
            borderwidth=0, highlightthickness=0,
        )
        self.overlays.add(row_iid, SLOT_TEXT_VALUE, lbl, place_text_value)
    ...
    def _insert_pair(
        ...
        first = items[0]
        pair_label = prop_row_label(
            first.get("row_label") or first.get("label", ""),
        )
    ...
    def _insert_prop(
        ...
        if prop.get("pair"):
            label = prop_row_label(prop.get("label") or pname)
        else:
