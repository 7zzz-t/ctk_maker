...
class WindowDuplicateDialog(ManagedToplevel):
    ...
    def __init__(
        ...
        if source_is_main:
            self.window_title = tr(
                "window_duplicate.title_window", "Duplicate Window",
            )
        else:
            self.window_title = tr(
                "window_duplicate.title_dialog", "Duplicate Dialog",
            )
    ...
    def build_content(self) -> ctk.CTkFrame:
        ...
        ctk.CTkLabel(
            body,
            text=tr(
                "window_duplicate.confirm_label",
                'Duplicate "{name}"?',
            ).format(name=self._source_name),
        ...
        ctk.CTkLabel(
            name_row, text=tr("window_duplicate.name_label", "Name"),
            font=ui_font(10),
        ...
        info_lines = [
            tr(
                "window_duplicate.info_placement",
        ...
        if self._source_is_main:
            info_lines.append(
                tr(
                    "window_duplicate.info_becomes_dialog",
        ...
        ctk.CTkButton(
            footer, text=tr("window_duplicate.duplicate_button", "Duplicate"),
            width=120, height=32,
            corner_radius=4, command=self._on_duplicate,
        ).pack(side="right")
        ctk.CTkButton(
            footer, text=tr("window_duplicate.cancel_button", "Cancel"),
            width=90, height=32,
            corner_radius=4,
