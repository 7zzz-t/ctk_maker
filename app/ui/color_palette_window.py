...
class ColorPaletteWindow(ManagedToplevel):
    ...
    def __init__(self, parent):
        ...
        self.window_title = tr("color.title", "Color Palette")
    ...
    def _build_header(self, parent: tk.Misc) -> None:
        ...
        stk.Label(
            bar, text=tr("color.title", "Color Palette"),
            bg=TOOLBAR_BG, fg=TREE_FG,
            font=ui_font(12, "bold"),
        ).pack(side="left", padx=12)
        stk.Label(
            bar, text=tr("color.hint", "click a swatch to copy hex"),
    ...
    def _build_palette_column(
        ...
        stk.Label(
            col, text=f"{pid} — {tr(f'color.palette.{pid}', plabel)}",
    ...
    def _build_color_card(
        ...
        name_lbl = stk.Label(
            text_frame, text=tr(f'color.name.{name}', name),
    ...
    def _copy(self, hex_code: str, name: str) -> None:
        ...
        self._status.configure(
            text=tr("color.copied", "  copied  {hex}  ({name})").format(hex=hex_code, name=tr(f'color.name.{name}', name)),
            fg=TREE_FG,
        )
