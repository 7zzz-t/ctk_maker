...
def show_crash_dialog(
    ...
    if not _parent_is_alive(parent):
        try:
            log_line = tr("crash.log_fallback", "Log: {path}").format(
                path=crash_log_path(),
            )
...
class _CrashDialog(ManagedToplevel):
    ...
    def build_content(self) -> ctk.CTkFrame:
        ...
        sub = ctk.CTkLabel(
            container,
            text=tr("crash.saved_to", "Saved to {path}").format(path=log_path),
            anchor="w",
        ...
        ctk.CTkButton(
            buttons, text=tr("crash.copy_traceback", "Copy traceback"), width=140,
            command=self._copy,
        ).pack(side="left")
        ctk.CTkButton(
            buttons, text=tr("crash.open_log_file", "Open log file"), width=140,
            command=lambda: _open_in_os(log_path),
        ).pack(side="left", padx=(8, 0))
        ctk.CTkButton(
            buttons, text=tr("crash.close", "Close"), width=100,
