...
class ProjectPanel(ctk.CTkFrame):
    ...
    def __init__(
        ...
        for evt in (
            ...
            # Document add/remove/rename changes the pages tree; the
            # asset tree was reading stale state until refresh fired.
    ...
    def refresh(self) -> None:
        ...
        if path:
            ...
            self._name_var.set(tr("project_window.untitled", "(untitled)"))
            self._path_var.set(tr("project_window.save_first", "save first"))
            try:
    ...
    def _on_show_add_menu(self) -> None:
        ...
        self._menu_command(
            menu, tr("project_window.menu.page", "Page..."), "app-window", self._on_new_page,
        )
        self._menu_command(menu, tr("project_window.menu.folder", "Folder"), "folder", self.files.on_new_folder)
        menu.add_separator()
        self._menu_command(
            menu, tr("project_window.menu.lucide", "Lucide Icon..."), "layout-list", self._on_add_lucide_icon,
        )
        self._menu_command(menu, tr("project_window.menu.image", "Image..."), "image", self._on_add_image)
        self._menu_command(menu, tr("project_window.menu.font", "Font..."), "type", self._on_add_font)
        menu.add_separator()
        self._menu_command(
            menu, tr("project_window.menu.python", "Python File (.py)"), "file-code",
            self.files.on_new_python_file,
        )
        self._menu_command(
            menu, tr("project_window.menu.text", "Text File (.md)"), "file-text", self.files.on_new_text_file,
        )
        try:
    ...
    def _build_add_submenu(self, parent_menu: tk.Menu) -> tk.Menu:
        ...
        self._menu_command(
            sub, tr("project_window.menu.lucide", "Lucide Icon..."), "layout-list", self._on_add_lucide_icon,
        )
        self._menu_command(
            sub, tr("project_window.menu.image", "Image..."), "image", self._on_add_image,
        )
        self._menu_command(
            sub, tr("project_window.menu.font", "Font..."), "type", self._on_add_font,
        )
        sub.add_separator()
        self._menu_command(
            sub, tr("project_window.menu.python", "Python File (.py)"), "file-code",
            self.files.on_new_python_file,
        )
        self._menu_command(
            sub, tr("project_window.menu.text", "Text File (.md)"), "file-text",
    ...
    def _on_add_image(self) -> None:
        self._add_asset(
            kind="images",
            title=tr("project_window.add_image_title", "Add image to project"),
            error_label=tr("project_window.image_label", "image"),
        )

    def _on_add_font(self) -> None:
        if not self._add_asset(
            kind="fonts",
            title=tr("project_window.add_font_title", "Add font to project"),
            error_label=tr("project_window.font_label", "font"),
        ):
            return
    ...
    def _add_asset(
        ...
        src = filedialog.askopenfilename(
            ...
            title=title,
            filetypes=[
                (tr(f"project_window.filetype.{kind}", filter_spec[0]), filter_spec[1]),
                (tr("project_window.filetype_all", "All files"), "*.*"),
            ],
        )
        if not src:
            return False
        ...
        if src_path.suffix.lower() not in exts:
            show_warning(
                tr("project_window.not_a_label", "Not a {label}").format(label=error_label),
                tr(
                    "project_window.not_a_label_body",
                    "{name} doesn't look like a {label} file.",
                ).format(name=src_path.name, label=error_label),
                parent=self.winfo_toplevel(),
            )
            return False
        ...
        try:
            ...
            show_error(
                tr("project_window.add_failed", "Add {label} failed").format(label=error_label),
                tr(
                    "project_window.add_failed_read",
                    "Couldn't read the {label} file.",
                ).format(label=error_label),
                parent=self.winfo_toplevel(),
            )
            return False
        ...
        if existing is None:
            ...
            try:
                ...
                show_error(
                    tr("project_window.add_failed", "Add {label} failed").format(label=error_label),
                    tr(
                        "project_window.add_failed_copy",
                        "Couldn't copy:\n{src}\n→\n{dst}",
                    ).format(src=src_path, dst=dst),
                    parent=self.winfo_toplevel(),
                )
                return False
...
class ProjectWindow(ManagedToplevel):
    ...
    def __init__(
        ...
        self.window_title = tr("project_window.title", "Assets")
