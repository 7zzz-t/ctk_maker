...
FILTER_ALL_LABEL = tr("tree_window.filter_all", "All types")
...
class ObjectTreePanel(ctk.CTkFrame):
    ...
    def _build_tree(self) -> None:
        ...
        search_entry = ctk.CTkEntry(
            filter_row,
            placeholder_text=tr("object_tree.search_placeholder", "Search by name…"),
        ...
        self.tree.heading("name", text=tr("object_tree.col.name", "Name"))
        self.tree.heading("type", text=tr("object_tree.col.type", "T"))
        self.tree.heading("layer", text=tr("object_tree.col.order", "Order"))
    ...
    def _refresh_doc_header(self) -> None:
        ...
        if active is None or self._doc_header_label is None:
            return
        label = active.name or tr("object_tree.untitled", "Untitled")
        if active.is_toplevel:
            label = f"{label}  ({tr('object_tree.dialog', 'Dialog')})"
    ...
    def _insert_document_row(self, doc, index: int) -> None:
        ...
        self.tree.insert(
            ...
            values=("", label, tr("object_tree.window", "Window"), str(index)),
            tags=("doc-row",),
        )
    ...
    def _insert_group_row(
        ...
        label = f"{INDENT_STR * depth}{arrow}◆ {tr('object_tree.group', 'Group')} ({len(members)})"
    ...
    def _on_right_click(self, event) -> str:
        ...
        if iid.startswith("group:"):
            ...
            menu.add_command(
                label=tr(
                    "object_tree.menu.select_group_count",
                    "Select Group ({count})",
            ...
            primary = next(iter(ids))
            menu.add_command(
                label=tr("object_tree.menu.ungroup", "Ungroup"),
        ...
        if multi_active:
            ...
            menu.add_command(
                label=tr(
                    "object_tree.menu.copy_widgets", "Copy {count} widgets",
                ).format(count=count),
            ...
            menu.add_command(
                label=tr(
                    "object_tree.menu.delete_widgets", "Delete {count} widgets",
                ).format(count=count),
            ...
            self.project.select_widget(iid)
            menu.add_command(
                label=tr("object_tree.menu.copy", "Copy"),
            ...
            menu.add_command(
                label=tr("object_tree.menu.paste", "Paste"),
                command=self._paste_in_window,
                foreground=paste_fg,
            )
            menu.add_command(
                label=tr("object_tree.menu.paste_as_child", "Paste as child"),
                command=lambda nid=iid: self._paste_as_child(nid),
                foreground=paste_child_fg,
            )
            menu.add_command(
                label=tr("object_tree.menu.duplicate", "Duplicate"),
                command=lambda nid=iid: self._duplicate_in_window(nid),
            )
            menu.add_separator()
            menu.add_command(
                label=tr("object_tree.menu.rename", "Rename"),
                command=lambda nid=iid: self._prompt_rename(nid),
            )
            menu.add_command(
                label=tr("object_tree.menu.delete", "Delete"),
                command=lambda nid=iid: self._delete_widget(nid),
            )
            self._add_group_entries(menu, toplevel)
            menu.add_separator()
            menu.add_command(
                label=tr("object_tree.menu.bring_to_front", "Bring to Front"),
                command=lambda nid=iid: self._z_order_with_history(
                    nid, "front",
                ),
            )
            menu.add_command(
                label=tr("object_tree.menu.send_to_back", "Send to Back"),
                command=lambda nid=iid: self._z_order_with_history(
                    nid, "back",
                ),
            )
    ...
    def _add_group_entries(self, menu, toplevel) -> None:
        ...
        if can_group:
            menu.add_command(
                label=tr("object_tree.menu.group", "Group"),
                accelerator=f"{MOD_LABEL_PLUS}G",
                command=toplevel._on_group_shortcut,
            )
        if select_group_id:
            menu.add_command(
                label=tr("object_tree.menu.select_group", "Select Group"),
    ...
    def _duplicate_in_window(self, widget_id: str) -> None:
        ...
        if entries:
            self.project.history.push(
                BulkAddCommand(
                    entries,
                    label=tr("tree_window.history_duplicate", "Duplicate"),
                ),
            )
    ...
    def _push_paste_history(self, new_ids: list[str]) -> None:
        ...
        if entries:
            self.project.history.push(
                BulkAddCommand(
                    entries,
                    label=tr("tree_window.history_paste", "Paste"),
                ),
            )
    ...
    def _delete_widget(self, widget_id: str) -> None:
        ...
        if node_check is not None and self._effective_locked(node_check):
            show_info(
                title=tr("object_tree.locked_title", "Widget locked"),
                message=tr(
                    "object_tree.locked_message",
        ...
        if widget_id in selected and len(selected) > 1:
            ...
            confirmed = ask_yes_no(
                title=tr("object_tree.delete_widgets_title", "Delete widgets"),
                message=tr(
                    "object_tree.delete_widgets_message",
                    "Delete {count} selected widgets?",
                ).format(count=count),
                parent=self.winfo_toplevel(),
                danger=True,
                yes_text=tr("object_tree.delete", "Delete"),
                no_text=tr("object_tree.cancel", "Cancel"),
            )
            if not confirmed:
                return
        ...
        confirmed = ask_yes_no(
            title=tr("object_tree.delete_widget_title", "Delete widget"),
            message=tr(
                "object_tree.delete_widget_message",
                "Delete this {type}?",
...
class ObjectTreeWindow(ManagedToplevel):
    ...
    def __init__(
        ...
        self.window_title = tr("object_tree.window_title", "Object Tree")
