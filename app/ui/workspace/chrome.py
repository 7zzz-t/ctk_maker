...
from app.ui.workspace.chrome_buttons import ChromeButtons
from app.ui.workspace.chrome_drag import ChromeDrag
from app.ui.workspace.chrome_removal import ChromeRemoval
...
class ChromeManager:
    ...
    def __init__(self, workspace) -> None:
        ...
        # Helper sidecars — each takes ``self`` (the chrome manager)
        # and reads/writes state through it. See module docstring +
        # the individual helper files for the surface each owns.
        self.buttons = ChromeButtons(self)
        self.drag = ChromeDrag(self)
        self.removal = ChromeRemoval(self)

    # ------------------------------------------------------------------
    # Pass-through shims — marquee.py + collapsed_tabs_bar.py reach
    # for these on the chrome manager directly.
    # ------------------------------------------------------------------
    def drive_drag(self, event) -> str:
        return self.drag.drive_drag(event)

    def end_drag(self, doc_id: str | None) -> str:
        return self.drag.end_drag(doc_id)

    def remove_document(self, doc_id: str) -> None:
        self.removal.remove_document(doc_id)
    ...
    def draw_for(self, doc) -> None:
        ...
        title_raw = str(doc.name or tr("chrome.untitled", "Untitled"))
        ...
        if not doc.is_toplevel:
            ...
            self.canvas.create_text(
                x_marker, mid,
                text=tr("workspace.chrome.my_windows", "[my windows]"),
                anchor="w",
                fill="#888888",
                font=ui_font(10),
    ...
    def _bind_for_document(
        ...
        for tag in (bg_tag, title_tag):
            self.canvas.tag_bind(
                tag, "<ButtonPress-1>",
                lambda e, d=doc_id: drag.on_press(e, d),
            )
            self.canvas.tag_bind(
                tag, "<B1-Motion>",
                lambda e, d=doc_id: drag.on_motion(e, d),
            )
            self.canvas.tag_bind(
                tag, "<ButtonRelease-1>",
                lambda e, d=doc_id: drag.on_release(e, d),
            )
            self.canvas.tag_bind(
                tag, "<Enter>",
                lambda _e: buttons.set_cursor("fleur"),
            )
            self.canvas.tag_bind(
                tag, "<Leave>",
                lambda _e: buttons.set_cursor(""),
            )
        ...
        self._bind_icon_hover(
            ...
            on_click=lambda d=doc_id: buttons.on_settings_click(None, d),
        )
        if (
            ...
            self._bind_icon_hover(
                ...
                on_click=lambda d=doc_id: buttons.on_minimize_click(d),
            )
        if (
            ...
            self._bind_icon_hover(
                ...
                on_click=lambda d=doc_id: buttons.on_desc_click(d),
            )
        if (
            ...
            self._bind_icon_hover(
                ...
                on_click=lambda d=doc_id: buttons.on_vars_click(
                    d, "local",
                ),
            )
        self._bind_icon_hover(
            ...
            on_click=lambda d=doc_id: buttons.on_tofront_click(d),
        )
        self._bind_icon_hover(
            toback_tag, toback_img_tag,
            self._toback_icon, self._toback_icon_hover,
            on_click=lambda d=doc_id: buttons.on_toback_click(d),
        )
        if (
            ...
            self._bind_icon_hover(
                ...
                on_click=lambda d=doc_id: buttons.on_preview_click(d),
            )
        if (
