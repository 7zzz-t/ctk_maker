...
class KeyboardActions:
    ...
    def on_delete(self, _event=None) -> str | None:
        ...
        if ws._effective_locked(sid):
            show_info(
                title=tr("keyboard.widget_locked", "Widget locked"),
                message=tr(
                    "keyboard.widget_locked_msg",
        ...
        confirmed = ask_yes_no(
            title=tr("keyboard.delete_widget", "Delete widget"),
            message=tr(
                "keyboard.delete_this", "Delete this {type}?",
            ).format(type=type_label),
            parent=ws.winfo_toplevel(),
            danger=True,
            yes_text=tr("keyboard.delete", "Delete"),
            no_text=tr("keyboard.cancel", "Cancel"),
        )
        if not confirmed:
            return "break"
    ...
    def _delete_multi(self, selected: set[str]) -> str:
        ...
        if locked_ids:
            show_info(
                title=tr("keyboard.widgets_locked", "Widgets locked"),
                message=tr(
                    "keyboard.widgets_locked_msg",
                    "{n} of the selected widgets are locked. "
                    "Unlock them from the Object Tree before deleting.",
                ).format(n=len(locked_ids)),
                parent=ws.winfo_toplevel(),
            )
            return "break"
        count = len(selected)
        confirmed = ask_yes_no(
            title=tr("keyboard.delete_widgets", "Delete widgets"),
            message=tr(
                "keyboard.delete_n_selected",
                "Delete {n} selected widgets?",
            ).format(n=count),
            parent=ws.winfo_toplevel(),
            danger=True,
            yes_text=tr("keyboard.delete", "Delete"),
