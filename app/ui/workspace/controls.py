...
class WorkspaceControls:
    ...
    def build_tool_bar(self) -> None:
        ...
        tools = [
            (TOOL_EDIT,   "vector-square",        tr("controls.edit_tooltip", "Edit (Q)")),
            (TOOL_SELECT, "mouse-pointer-2",      tr("controls.select_tooltip", "Select (W)")),
            (TOOL_HAND,   "hand",                 tr("controls.hand_tooltip", "Hand (E)")),
        ]
        ...
        _attach_tooltip(self._btn_preview, tr("controls.preview_project_tooltip", "Preview Project (Ctrl+R / F5)"))
        ...
        _attach_tooltip(self._btn_preview_active, tr("controls.preview_active_tooltip", "Preview Active Dialog (Ctrl+P)"))

        self._lbl_preview_current = ctk.CTkLabel(
            group, text=tr("controls.no_active_dialog", "No active dialog"),
        ...
        def _show_forms_menu(event=None, btn=None):
            ...
            for doc in self.project.documents:
                label = doc.name or tr("controls.untitled", "Untitled")
                if doc.is_toplevel:
                    label = f"{label}  {tr('controls.dialog_suffix', '(Dialog)')}"
            ...
            if visible_count >= 2:
                menu.add_separator()
                menu.add_command(
                    label="   " + tr("controls.arrange_horizontal", "Arrange Horizontally"),
                    command=lambda: self._arrange_documents("horizontal"),
                )
                menu.add_command(
                    label="   " + tr("controls.arrange_vertical", "Arrange Vertically"),
        ...
        forms_btn = ctk.CTkButton(
            bar, text=tr("controls.all_windows", "All Windows"),
        ...
        add_btn = ctk.CTkButton(
            bar,
            text=tr("controls.add_dialog", "Add Dialog"),
            image=plus_icon,
            compound="left",
            width=110,
            height=24,
    ...
    def refresh_preview_buttons(self) -> None:
        ...
        if self._lbl_preview_current is not None:
            if is_dialog:
                name = (doc.name or tr("controls.untitled", "Untitled")).strip()
                ...
                self._lbl_preview_current.configure(
                    text=tr("controls.no_active_dialog", "No active dialog"),
                    text_color=PREVIEW_LABEL_OFF,
                )
