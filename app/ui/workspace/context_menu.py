...
class ContextMenu:
    ...
    def on_canvas_right_click(self, event) -> str:
        ...
        menu.add_cascade(label=tr("ctx_menu.add_widget", "Add Widget"), menu=add_submenu)
        menu.add_separator()
        paste_state = "normal" if ws.project.clipboard else "disabled"
        menu.add_command(
            label=tr("ctx_menu.paste", "Paste"),
        ...
        menu.add_command(
            label=tr("ctx_menu.select_all", "Select All"),
        ...
        menu.add_command(
            label=tr("ctx_menu.deselect_all", "Deselect All"),
            command=lambda: ws.project.select_widget(None),
            state=deselect_state,
        )
        menu.add_separator()
        save_label = (
            tr("ctx_menu.save_dialog_as_component", "Save Dialog as Component")
            if doc.is_toplevel else tr("ctx_menu.save_window_as_component", "Save Window as Component")
        )
        ...
        dup_label = (
            tr("ctx_menu.duplicate_dialog", "Duplicate Dialog")
            if doc.is_toplevel else tr("ctx_menu.duplicate_window", "Duplicate Window")
        )
        menu.add_command(
            label=dup_label,
        ...
        menu.add_separator()
        menu.add_command(
            label=tr("ctx_menu.minimize_window", "Minimize Window"),
        ...
        props_label = (
            tr("ctx_menu.dialog_properties", "Dialog Properties")
            if doc.is_toplevel else tr("ctx_menu.window_properties", "Window Properties")
        )
        menu.add_command(
            label=props_label,
    ...
    def _paste_at_canvas(self, doc, logical_x: int, logical_y: int) -> None:
        ...
        if entries:
            ws.project.history.push(
                BulkAddCommand(
                    entries,
                    label=tr("workspace.ctx_menu.history_paste", "Paste"),
                ),
            )
    ...
    def on_widget_right_click(self, event, nid: str) -> str:
        ...
        if multi_active:
            ...
            menu.add_command(
                label=tr("ctx_menu.copy_n", "Copy {n} widgets").format(n=count),
                command=self._copy_selection,
            )
            menu.add_command(
                label=tr("ctx_menu.duplicate_n", "Duplicate {n} widgets").format(n=count),
                command=self._duplicate_selection,
            )
            menu.add_separator()
            menu.add_command(
                label=tr("ctx_menu.delete_n", "Delete {n} widgets").format(n=count),
                command=ws._on_delete,
            )
            menu.add_separator()
            menu.add_command(
                label=tr("ctx_menu.save_n_as_component", "Save {n} widgets as component…").format(n=count),
            ...
            menu.add_cascade(
                label=tr("ctx_menu.add_widget_as_child", "Add Widget as Child"),
                menu=add_submenu,
                state="normal" if target_is_container else "disabled",
            )
            menu.add_separator()
            menu.add_command(
                label=tr("ctx_menu.copy", "Copy"),
            ...
            menu.add_command(
                label=tr("ctx_menu.duplicate", "Duplicate"),
            ...
            menu.add_command(
                label=tr("ctx_menu.edit_mode", "Edit mode"),
                command=lambda: self._enter_edit_mode(nid),
                state=edit_state,
            )
            menu.add_command(
                label=tr("ctx_menu.description", "Description…"),
                command=lambda: self._open_widget_description(nid),
            )
            menu.add_separator()
            handler_submenu = self._build_handler_menu(target_node)
            menu.add_cascade(
                label=tr("ctx_menu.add_handler", "Add handler"),
                menu=handler_submenu,
                state=("normal" if handler_submenu is not None else "disabled"),
            )
            menu.add_separator()
            menu.add_command(
                label=tr("ctx_menu.save_as_component", "Save as component…"),
                command=self._save_selection_as_component,
            )
            menu.add_separator()
            menu.add_command(
                label=tr("ctx_menu.rename", "Rename"),
                command=lambda: self._prompt_rename_widget(nid),
            )
            menu.add_command(label=tr("ctx_menu.delete", "Delete"), command=ws._on_delete)
            self._add_group_entries_to_menu(menu, toplevel)
            menu.add_separator()
            menu.add_command(
                label=tr("ctx_menu.bring_to_front", "Bring to Front"),
                command=lambda: self._z_order_with_history(nid, "front"),
            )
            menu.add_command(
                label=tr("ctx_menu.send_to_back", "Send to Back"),
    ...
    def _duplicate_selection(self) -> None:
        ...
        if entries:
            ws.project.history.push(
                BulkAddCommand(
                    entries,
                    label=tr("workspace.ctx_menu.history_duplicate", "Duplicate"),
                ),
            )
    ...
    def _build_handler_menu(self, node) -> "tk.Menu | None":
            ...
            for entry in entries:
                ...
                if methods:
                    for method in methods:
                        ...
                        shown = f"{cls}.{m}" if cls and m else (
                            cls or tr("workspace.ctx_menu.script", "script")
                        )
                    ...
                    menu.add_command(
                        label=tr(
                            "workspace.ctx_menu.add_another_action",
                            "+  Add another {event} action",
                        ).format(event=entry.label.lower()),
        ...
        if advanced_events:
            ...
            if default_events:
                sub.add_separator()
            sub.add_cascade(label=tr("ctx_menu.advanced", "Advanced"), menu=adv)
        return sub
    ...
    def _attach_event_handler(self, widget_id: str, event_key: str) -> None:
        ...
        if not getattr(ws.project, "path", None):
            show_info(
                tr("ctx_menu.save_first", "Save first"),
                tr("ctx_menu.save_first_msg", "Save the project before adding event handlers — the scripts/ folder lives in the project folder."),
    ...
    def _save_selection_as_component(self) -> None:
        ...
        if components_dir is None:
            show_info(
                tr("ctx_menu.save_project_first", "Save project first"),
                tr("ctx_menu.save_project_first_msg", "Components are stored next to assets in the project folder. Save the project before creating components."),
                parent=toplevel,
            )
            return
        ...
        try:
            ...
            show_error(
                tr("ctx_menu.save_component_failed", "Save component failed"),
                tr("ctx_menu.couldnt_write_component", "Couldn't write component:\n{exc}").format(exc=exc),
                parent=toplevel,
            )
            return
    ...
    def _save_window_as_component(self, document) -> None:
        ...
        if not document.root_widgets:
            show_info(
                tr("workspace.ctx_menu.empty_window_title", "Empty window"),
                tr(
                    "workspace.ctx_menu.empty_window_msg",
