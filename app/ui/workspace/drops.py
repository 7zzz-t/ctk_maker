...
class DropDispatcher:
    ...
    def on_component_drop(
        ...
        if entries:
            label = payload.get("name") or tr(
                "workspace.drops.component_fallback", "component",
            )
            ...
            ws.project.history.push(
                BulkAddCommand(
                    entries,
                    label=tr(
                        "workspace.drops.insert_component",
                        "Insert {name}",
                    ).format(name=label),
                ),
            )
    ...
    def _build_add_widget_menu(
        ...
        for group in CATALOG:
            ...
            submenu.add_cascade(
                label=tr(f"palette.group.{group.title}", group.title),
                menu=group_menu,
            )
        return submenu
