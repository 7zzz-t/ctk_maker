...
class Renderer:
    ...
    def _draw_ghost_statusbar(
        ...
        * tooltip — surfaces the *why* on hover (performance gain +
        ...
        label = (
            tr(
                "workspace.render.ghost_statusbar_on",
                "● GHOST  —  click to restore live widgets",
            )
            if is_ghost
            else tr(
                "workspace.render.ghost_statusbar_off",
                "● Live  —  click to ghost (screenshot mode)",
            )
        )
        sb_top = y2
    ...
    def _show_statusbar_tooltip(
        ...
        if is_ghost:
            text = tr(
                "workspace.render.ghost_tooltip_on",
            ...
            text = tr(
                "workspace.render.ghost_tooltip_off",
