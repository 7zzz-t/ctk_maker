...
def ghost_all_live_docs(workspace: "Workspace") -> None:
    ...
    zoom.begin_batch(
        "      " + tr(
            "workspace.bulk_ghost.preparing",
            "Ghosting 0 / {total} — preparing...  (Esc to cancel)",
        ).format(total=len(targets)),
        colour,
    )
    root.update()
    ...
    try:
        for i, doc in enumerate(targets):
            ...
            zoom.set_batch_text(
                "      " + tr(
                    "workspace.bulk_ghost.progress",
                    "Ghosting {index} / {total} — {doc_name}  (Esc to cancel)",
                ).format(
                    index=i + 1,
                    total=len(targets),
                    doc_name=doc.name[:40],
                ),
            )
            root.update()
    finally:
