...
CURRENT_MARKER_KEY = "history.current_state"
EMPTY_TEXT_KEY = "history.empty"
...
class HistoryPanel(ctk.CTkFrame):
    ...
    def _refresh(self) -> None:
        ...
        if not undo_stack and not redo_stack:
            self.tree.insert("", "end", text=tr(EMPTY_TEXT_KEY, "(no history yet)"), tags=("empty",))
            return
        ...
        self.tree.insert(
            "", "end", iid="current",
            text=tr(CURRENT_MARKER_KEY, "◉ Current state"), tags=("current",),
        )
...
class HistoryWindow(ManagedToplevel):
    ...
    def __init__(
        ...
        self.project = project
        self.window_title = tr("history.title", "History")
