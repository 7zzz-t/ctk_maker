...
def _project_file_types():
    return [
        (tr("main_files.file_type_project", "CTkMaker project"), "*.ctkproj"),
        (tr("main_files.file_type_all", "All files"), "*.*"),
    ]
...
class FilesMixin(_MainWindowHost):
        ...
        try:
            ...
            if result[0] == "open":
                ...
                try:
                    ...
                    show_error(
                        tr("main_files.save_failed_title", "Save failed"),
                        tr("main_files.create_project_failed_msg",
                           "Could not create project file at:\n{path}").format(path=path),
                        parent=self,
                    )
                    return
                clear_autosave(path)
    ...
    def _open_path(self, path: str) -> None:
        if not Path(path).exists():
            show_error(
                tr("main_files.open_failed_title", "Open failed"),
                tr("main_files.file_not_found_msg", "File not found:\n{path}").format(path=path),
                parent=self,
            )
            return
        ...
        try:
            ...
            show_crash_dialog(
                self, tr("main_files.open_failed_title", "Open failed"),
                tr("main_files.unexpected_open_error_msg",
    ...
    def _maybe_swap_for_autosave(self, path: str) -> str:
        ...
        choice = ask_yes_no(
            tr("main_files.restore_autosave_title", "Restore from autosave?"),
            tr(
                "main_files.restore_autosave_msg",
                "An autosave from {ts} is newer than the saved "
                "project file.\n\n"
                "This usually means the previous session ended "
                "without an explicit Save — for example after a "
                "crash or a forced close.\n\n"
                "Yes  → restore from the autosave\n"
                "No   → open the older saved copy "
                "(autosave is left untouched)",
            ).format(ts=ts),
            parent=self,
        )
    ...
    def _stub(self, name: str) -> None:
        show_info(
            tr("main_files.toolbar_stub_title", "Toolbar stub"),
            tr("main_files.toolbar_stub_msg",
               "{name} — not implemented yet").format(name=name),
            parent=self,
        )
    ...
    def _on_recover_from_backup(self) -> None:
        ...
        path = filedialog.askopenfilename(
            parent=self,
            title=tr("main_files.recover_backup_title", "Recover project from backup"),
            filetypes=[
                (tr("main_files.file_type_backup", "CTkMaker backup"), "*.ctkproj.bak"),
                (tr("main_files.file_type_all", "All files"), "*.*"),
            ],
        )
        if not path:
        ...
        if not Path(path).exists():
            show_error(
                tr("main_files.recover_failed_title", "Recover failed"),
                tr("main_files.file_not_found_msg", "File not found:\n{path}").format(path=path),
                parent=self,
            )
            return
        ...
        try:
            ...
            show_crash_dialog(
                self, tr("main_files.recover_failed_title", "Recover failed"),
                tr("main_files.unexpected_recover_error_msg",
        ...
        show_info(
            tr("main_files.recovered_title", "Recovered from backup"),
            tr("main_files.recovered_msg",
    ...
    def _maybe_prompt_legacy_convert(self) -> None:
        ...
        """
        choice = ask_yes_no(
            tr("main_files.convert_title", "Convert to multi-page project?"),
            tr(
                "main_files.convert_msg",
    ...
    def _do_legacy_convert(self) -> None:
        ...
        try:
            ...
            show_error(
                tr("main_files.convert_failed_title", "Convert failed"),
                tr("main_files.convert_pre_save_failed_msg",
        ...
        self._show_toast(tr("main_files.converted_toast", "Converted to multi-page project"))
    ...
    def _switch_to_page(self, page_path: str) -> bool:
        ...
        if not target.exists():
            show_error(
                tr("main_files.switch_failed_title", "Switch failed"),
                tr("main_files.page_not_found_msg", "Page file not found:\n{path}").format(path=target),
                parent=self,
            )
            return False
        if not self._confirm_discard_if_dirty():
        ...
        self._show_toast(
            tr("main_files.switched_toast", "Switched to: {name}").format(name=switched_name),
        )
        return True
    ...
    def _on_save(self) -> None:
        if self._current_path:
            try:
                ...
                show_error(
                    tr("main_files.save_failed_title", "Save failed"),
                    tr("main_files.write_failed_msg",
    ...
    def _on_save_as(self) -> None:
        ...
        path = filedialog.asksaveasfilename(
            parent=self,
            title=tr("main_files.save_project_as_title", "Save project as"),
            defaultextension=".ctkproj",
            filetypes=_project_file_types(),
        )
        if not path:
            return
    ...
    def _save_as_new_page(self, name: str) -> None:
        ...
        if folder_path is None:
            show_error(
                tr("main_files.save_failed_title", "Save failed"),
                tr("main_files.project_folder_not_set_msg",
        ...
        try:
            ...
            show_error(
                tr("main_files.save_failed_title", "Save failed"),
                tr("main_files.write_new_page_failed_msg",
        ...
        self._show_toast(
            tr("main_files.saved_as_toast", "Saved as: {name}").format(name=name),
        )
    ...
    def _save_as_clone_project(self, name: str, save_to: str) -> None:
        ...
        if self._current_path is None:
            show_error(
                tr("main_files.save_failed_title", "Save failed"),
                tr("main_files.no_project_open_msg",
        ...
        try:
            ...
            show_error(
                tr("main_files.save_failed_title", "Save failed"),
                tr("main_files.clone_pre_save_failed_msg",
        ...
        self._show_toast(
            tr("main_files.cloned_toast", "Cloned to: {name}").format(name=name),
        )
    ...
    def _save_as_extract_page(self, name: str, save_to: str) -> None:
        ...
        try:
            ...
            show_error(
                tr("main_files.save_failed_title", "Save failed"),
                tr("main_files.extract_pre_save_failed_msg",
        ...
        self._open_path(str(new_page_path))
        self._show_toast(
            tr("main_files.extracted_toast", "Extracted to: {name}").format(name=name),
        )
