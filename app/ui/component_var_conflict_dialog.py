...
class ComponentVarConflictDialog(ManagedToplevel):
    ...
    def __init__(self, parent, conflicts: "list[VarConflict]"):
        self.window_title = tr("var_conflict.title", "Resolve variable conflicts")
    ...
    def build_content(self) -> ctk.CTkFrame:
        ...
        intro = ctk.CTkLabel(
            container,
            text=tr("var_conflict.intro", "These variables already exist in the target window with a different type. Pick a resolution for each:"),
            font=ui_font(10),
            text_color="#cccccc",
            wraplength=420,
            justify="left",
        ...
        ctk.CTkButton(
            footer, text=tr("var_conflict.ok", "OK"), width=100, height=30,
            corner_radius=4, command=self._on_ok,
        ).pack(side="right")
        ctk.CTkButton(
            footer, text=tr("var_conflict.cancel", "Cancel"), width=80, height=30,
            corner_radius=4,
    ...
    def _build_row(self, parent, conflict: "VarConflict") -> None:
        ...
        ctk.CTkLabel(
            wrap,
            text=tr(
                "var_conflict.type_row",
                "component: {comp}    existing: {existing}",
            ).format(comp=comp_type, existing=existing_type),
            font=ui_font(9),
            text_color="#888888",
            anchor="w",
        ...
        ctk.CTkRadioButton(
            row1, text=tr("var_conflict.rename", "Rename to"), variable=choice_var, value="rename",
            font=ui_font(10),
        ).pack(side="left")
        ...
        ctk.CTkRadioButton(
            row2, text=tr("var_conflict.skip", "Skip binding"), variable=choice_var, value="skip",
            font=ui_font(10),
        ).pack(side="left")
        ...
        for state in self._row_state:
            ...
            if choice == "rename":
                if not _is_valid_name(new_name):
                    self.bell()
                    show_warning(
                        tr("var_conflict.invalid_title", "Invalid name"),
                        tr("var_conflict.invalid_msg", "'{name}' isn't a valid variable name.").format(name=new_name),
                        parent=self,
                    )
                    return
