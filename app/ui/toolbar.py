...
class Toolbar(ctk.CTkFrame):
    def __init__(
        ...
        *,
        ...
        self._add_button("file-plus", on_new, tooltip=tr("toolbar.new", "New project"))
        self._add_button("folder", on_open, tooltip=tr("toolbar.open", "Open project"))
        self._add_button("save", on_save, tooltip=tr("toolbar.save", "Save"))
        self._add_separator()
        self._add_button(
            "file-code", on_export, tooltip=tr("toolbar.export", "Export to Python"),
            color=ICON_TINT_CARROT,
        )
        if on_run_script is not None:
            self._add_button(
                "tv-minimal-play", on_run_script,
                tooltip=tr("toolbar.run_script", "Run Python Script..."),
                color=ICON_TINT_CARROT,
            )
        self._add_separator()
        ...
        self._undo_button = self._add_toggle_button(
            self._icon_undo_off, on_undo, tooltip=tr("toolbar.undo", "Undo (Ctrl+Z)"),
        )
        self._redo_button = self._add_toggle_button(
            self._icon_redo_off, on_redo, tooltip=tr("toolbar.redo", "Redo (Ctrl+Y)"),
        )
    ...
    def _build_align_group(self, on_align: Callable[[str], None]) -> None:
        ...
        specs: list[tuple[str, str, str]] = [
            (MODE_LEFT, "align-start-vertical", tr("toolbar.align_left", "Align Left")),
            (MODE_CENTER_H, "align-center-vertical", tr("toolbar.align_center_h", "Align Center (Horizontal)")),
            (MODE_RIGHT, "align-end-vertical", tr("toolbar.align_right", "Align Right")),
            (MODE_TOP, "align-start-horizontal", tr("toolbar.align_top", "Align Top")),
            (MODE_CENTER_V, "align-center-horizontal", tr("toolbar.align_center_v", "Align Middle (Vertical)")),
            (MODE_BOTTOM, "align-end-horizontal", tr("toolbar.align_bottom", "Align Bottom")),
        ]
        ...
        for mode, icon_name, tooltip in (
            (MODE_DISTRIBUTE_H, "align-horizontal-distribute-center",
             tr("toolbar.distribute_h", "Distribute Horizontally")),
            (MODE_DISTRIBUTE_V, "align-vertical-distribute-center",
             tr("toolbar.distribute_v", "Distribute Vertically")),
        ):
            btn = self._add_align_button(icon_name, mode, on_align, tooltip)
            self._align_buttons[mode] = btn
            self._align_enabled[mode] = False
    ...
    def _add_report_bug_button(
        ...
        btn = ctk.CTkButton(
            self,
            text=tr("toolbar.report_bug", "Bug Report / Feature Request"),
            image=icon,
            width=220,
            height=28,
            corner_radius=4,
        ...
        _attach_tooltip(btn, tr("toolbar.report_bug_tooltip", "Report a bug or request a feature"))
        return btn
