...
class TransitionsDemoWindow(ManagedToplevel):
    ...
    def build_content(self) -> ctk.CTkFrame:
        self.window_title = tr("transitions.title", "Transitions Demo")
        ...
        tab_names = (
            tr("transitions.tab_button", "Button"),
            tr("transitions.tab_text", "Text"),
            tr("transitions.tab_card", "Card"),
            tr("transitions.tab_loaders", "Loaders"),
            tr("transitions.tab_popups", "Popups"),
            tr("transitions.tab_toasts", "Toasts"),
        )
    ...
    def _refresh_selection(self):
        ...
        self.selection_label.configure(
            text=tr(
                "transitions_demo.selected_status",
                "Selected: {name} ({category})    "
                "Easing={easing}    Duration={duration}s",
            ).format(
    ...
    def _build_stage(self, parent):
        ...
        ctk.CTkLabel(
            self.stage, text=tr("transitions.click_button_animate_card", "Click a button to animate the card."),
        ...
        ctk.CTkLabel(
            self.card, text=tr("transitions_demo.target", "Target"),
            font=ui_font(13, "bold"), text_color="white",
    ...
    def _add_loaders_instruction(self):
        ctk.CTkLabel(
            self.loaders_stage,
            text=tr(
                "transitions_demo.loaders_instruction",
                "Click a button to see a loading indicator.",
            ),
            font=ui_font(BUTTON_FONT_SIZE), text_color=EMPTY_FG,
        ).place(relx=0.5, rely=0.5, anchor="center")
    ...
    def _build_btn_stage(self, parent):
        ...
        ctk.CTkLabel(
            self.btn_stage,
            text=tr(
                "transitions_demo.btn_instruction",
                "Pick an effect, then click 'Press me'.",
            ),
            font=ui_font(BUTTON_FONT_SIZE), text_color=EMPTY_FG,
        ).place(relx=0.5, rely=0.06, anchor="n")
        ...
        self.sample_btn = ctk.CTkButton(
            self.btn_stage,
            text=tr("transitions_demo.press_me", "Press me"),
    ...
    def _build_toasts_tab(self, parent):
        ...
        ctk.CTkLabel(
            stage,
            text=tr(
                "transitions_demo.toasts_instruction",
        ...
        row.pack(fill="x", padx=CONTENT_PADX, pady=8)
        ctk.CTkLabel(
            row, text=tr("transitions.position", "Position:"), font=ui_font(BUTTON_FONT_SIZE),
            text_color=TREE_FG,
    ...
    def _build_button_tab(self, parent):
        ...
        row.pack(fill="x", padx=CONTENT_PADX, pady=8)
        ctk.CTkLabel(
            row, text=tr("transitions_demo.press_effect", "Press effect:"),
            font=ui_font(BUTTON_FONT_SIZE), text_color=TREE_FG,
        ).pack(side="left", padx=(0, 6))
        ctk.CTkOptionMenu(
            row, values=[
        ...
        ctk.CTkLabel(
            row,
            text=tr("transitions_demo.hover_effect", "Hover effect:"),
            font=ui_font(BUTTON_FONT_SIZE), text_color=TREE_FG,
        ).pack(side="left", padx=(0, 6))
        ctk.CTkOptionMenu(
    ...
    def _build_common_controls(self, parent):
        ...
        ctk.CTkLabel(
            row, text=tr("transitions_demo.easing", "Easing:"),
            font=ui_font(BUTTON_FONT_SIZE), text_color=TREE_FG,
        ...
        ctk.CTkLabel(
            row, text=tr("transitions.duration", "Duration (s):"), font=ui_font(BUTTON_FONT_SIZE),
            text_color=TREE_FG,
        ).pack(side="left", padx=(0, 4))
        ctk.CTkEntry(
        ...
        primary_button(
            row, tr("transitions.generate_code", "Generate code"), self._show_generated_code, width=140,
        ).pack(side="right")
    ...
    def _reset_text_stage(self, text=None):
        ...
        self.text_label = ctk.CTkLabel(
            self.text_stage,
            text=(
                tr("transitions_demo.text_default", self.DEFAULT_TEXT)
    ...
    def demo_text_typewriter(self):
        text = tr("transitions_demo.typewriter_text", self.TYPEWRITER_TEXT)
    ...
    def demo_text_wave(self):
        text = tr("transitions_demo.wave_text", self.WAVE_TEXT)
        self._clear_text_stage()
    ...
    def demo_text_fade_words(self):
        text = tr("transitions_demo.fade_words_text", self.FADE_WORDS_TEXT)
        self._clear_text_stage()
    ...
    def demo_toast_success(self):
        self._make_toast(
            tr(
                "transitions_demo.toast_success",
    ...
    def demo_toast_warning(self):
        self._make_toast(
            tr(
                "transitions_demo.toast_warning",
    ...
    def demo_popup_fade(self):
        top, tw, th, tx, ty = self._make_popup(
            tr("transitions_demo.popup_fade", "Fade"),
        )
    ...
    def demo_popup_pop(self):
        top, tw, th, tx, ty = self._make_popup(
            tr("transitions_demo.popup_pop", "Pop"),
        )
    ...
    def demo_popup_slide_down(self):
        top, tw, th, tx, ty = self._make_popup(
            tr("transitions_demo.popup_slide_down", "Slide-Down"),
        )
        start_y = ty - 240
    ...
    def demo_popup_slide_up(self):
        top, tw, th, tx, ty = self._make_popup(
            tr("transitions_demo.popup_slide_up", "Slide-Up"),
        )
        start_y = ty + 240
        top.geometry(f"{tw}x{th}+{tx}+{start_y}")
    ...
    def demo_popup_slide_right(self):
        top, tw, th, tx, ty = self._make_popup(
            tr("transitions_demo.popup_slide_right", "Slide-Right"),
        )
        start_x = tx - 300
    ...
    def demo_popup_zoom(self):
        top, tw, th, tx, ty = self._make_popup(
            tr("transitions_demo.popup_zoom", "Zoom"),
        )
    ...
    def _show_generated_code(self):
        ...
        top = ctk.CTkToplevel(self)
        top.title(tr("transitions_demo.generated_code_title", "Generated code"))
        ...
        copy_text = tr(
            "transitions_demo.copy_to_clipboard", "Copy to clipboard",
        )
        copy_btn = primary_button(
            btn_row, copy_text, lambda: None, width=160,
        )

        def copy():
            top.clipboard_clear()
            top.clipboard_append(code)
            copy_btn.configure(text=tr("transitions_demo.copied", "Copied!"))
            top.after(1200, lambda: copy_btn.configure(text=copy_text))

        copy_btn.configure(command=copy)
        copy_btn.pack(side="left")
        secondary_button(
            btn_row, tr("transitions_demo.close", "Close"),
            top.destroy, width=80,
        ).pack(side="right")
