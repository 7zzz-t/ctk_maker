...
class ConsolePanel(ctk.CTkFrame):
    ...
    def _build(self) -> None:
        ...
        copy_btn = ctk.CTkButton(
            self._toolbar, text=tr("console.copy", "Copy"),
        ...
        _attach_tooltip(copy_btn, tr("console.copy_tooltip", "Copy all to clipboard"))
        ...
        _attach_tooltip(search_btn, tr("console.search_tooltip", "Search (Ctrl+F)"))
        ...
        for key, label, show_tooltip in (
            ("info", tr("console.filter_info", "Info"),
             tr("console.show_info_messages", "Show info messages")),
            ("warning", tr("console.filter_warn", "Warn"),
             tr("console.show_warn_messages", "Show warn messages")),
            ("error", tr("console.filter_error", "Error"),
             tr("console.show_error_messages", "Show error messages")),
            ("debug", tr("console.filter_debug", "Debug"),
             tr("console.show_debug_messages", "Show debug messages")),
        ):
            ...
            _attach_tooltip(cb, show_tooltip)
        ...
        clear_btn = ctk.CTkButton(
            self._toolbar, text=tr("console.clear", "Clear"),
        ...
        stop_btn = ctk.CTkButton(
            self._toolbar, text=tr("console.stop", "Stop"),
        ...
        if self._clear_on_preview_var is not None:
            auto_clear_cb = ctk.CTkCheckBox(
                self._toolbar,
                text=tr("console.auto_clear", "Auto-clear on preview"),
        ...
        lock_cb = ctk.CTkCheckBox(
            self._toolbar, text=tr("console.lock_scroll", "Lock scroll"),
        ...
        if self._on_close is not None:
            ...
            _attach_tooltip(close_btn, tr("console.close_tooltip", "Close"))
            lock_cb.pack(
        ...
        _attach_tooltip(lock_cb, tr("console.lock_tooltip", "Don't auto-scroll on new output"))
        if auto_clear_cb is not None:
            auto_clear_cb.pack(
                side="right", padx=(0, 4), pady=CONSOLE_TOOLBAR_PADY,
            )
            _attach_tooltip(auto_clear_cb, tr("console.auto_clear_tooltip", "Clear log on each preview start"))
        stop_btn.pack(
            side="right", padx=(0, 4), pady=CONSOLE_TOOLBAR_PADY,
        )
        _attach_tooltip(stop_btn, tr("console.stop_tooltip", "Stop preview process"))
        clear_btn.pack(
            side="right", padx=(0, 4), pady=CONSOLE_TOOLBAR_PADY,
        )
        _attach_tooltip(clear_btn, tr("console.clear_tooltip", "Clear log"))
    ...
    def _render_entry(self, stream: str, ts: str, line: str) -> None:
        ...
        if truncated_extra > 0:
            suffix = tr(
                "console.truncated_suffix",
                "truncated, +{n} chars",
            ).format(n=truncated_extra)
            self._text.insert(
                "end",
                f"  … [{suffix}]",
                ("truncated",) + body_tags,
    ...
    def _build_context_menu(self) -> None:
        ...
        menu.add_command(
            label=tr("console.menu_copy", "Copy"),
            command=self._handle_copy,
        )
        menu.add_command(
            label=tr("console.menu_select_all", "Select all"),
            command=self._handle_select_all,
        )
        menu.add_separator()
        menu.add_command(
            label=tr("console.menu_clear", "Clear"),
            command=self._handle_clear,
        )
        menu.add_separator()
        menu.add_command(
            label=tr("console.menu_palette_test", "Emit colour test palette"),
    ...
    def _emit_palette_test(self) -> None:
        ...
        logger.debug(tr("console.palette_debug", "debug message — dim gray"))
        logger.info(tr("console.palette_info", "info message — default foreground"))
        logger.warning(tr("console.palette_warning", "warning message — amber"))
        logger.error(tr("console.palette_error", "error message — red"))
        logger.critical(tr("console.palette_critical", "critical message — bold red"))
    ...
    def _build_search_bar(self, parent) -> None:
        ...
        entry = style.styled_entry(
            ...
            placeholder_text=tr("console.find_placeholder", "Find…"),
        )
...
class ConsoleWindow(ManagedToplevel):
    ...
    def __init__(
        ...
        self.window_title = tr("console.title", "Console")
