...
class ImagePickerDialog(ManagedToplevel):
    ...
    def __init__(self, parent, project_file: str, event_bus=None):
        ...
        self.window_title = tr("image_picker.title", "Select image")
    ...
    def _build_header(self, parent) -> None:
        ...
        style.primary_button(
            bar, tr("image_picker.import_image", "+ Import image..."),
            command=self._on_import, width=140,
        ).pack(side="left", padx=(10, 4), pady=10)
        style.secondary_button(
            bar, tr("image_picker.import_lucide", "+ Lucide icon..."),
    ...
    def _build_footer(self, parent) -> None:
        ...
        self._ok_btn = style.primary_button(
            foot, tr("image_picker.ok", "OK"), command=self._on_ok, width=90,
        )
        self._ok_btn.configure(state="disabled")
        self._ok_btn.pack(side="right")
        style.secondary_button(
            foot, tr("image_picker.cancel", "Cancel"),
    ...
    def _refresh(self, select: Path | None = None) -> None:
        ...
        if not images:
            ctk.CTkLabel(
                self._list_wrap,
                text=tr("image_picker.empty", "No images yet. Click + Import to add one."),
    ...
    def _on_row_right_click(self, event, path: Path) -> None:
        ...
        menu.add_command(
            label=tr(
                "image_picker.remove_from_project",
    ...
    def _remove_image(self, path: Path) -> None:
        ...
        """
        if not ask_yes_no(
            tr("image_picker.remove_title", "Remove from project"),
            tr(
                "image_picker.remove_message",
            ...
            ).format(name=path.name, path=path),
            parent=self,
            danger=True,
            yes_text=tr("image_picker.remove_yes", "Remove"),
            no_text=tr("image_picker.cancel", "Cancel"),
        ):
            return
        try:
            if path.exists():
            ...
            show_error(
                tr("image_picker.remove_failed_title", "Remove failed"),
                tr("image_picker.remove_failed_msg", "Couldn't delete:\n{path}").format(path=path),
                parent=self,
            )
            return
    ...
    def _on_import(self) -> None:
        src = filedialog.askopenfilename(
            parent=self,
            title=tr("image_picker.import_dialog_title", "Import image into project"),
            filetypes=[
                (tr("image_picker.image_files", "Image files"),
                 "*.png *.jpg *.jpeg *.gif *.bmp *.webp *.ico"),
                (tr("image_picker.all_files", "All files"), "*.*"),
            ],
        )
        if not src:
            return
        if Path(src).suffix.lower() not in IMAGE_EXTS:
            show_warning(
                tr("image_picker.not_image_title", "Not an image"),
                tr(
                    "image_picker.not_image_msg",
                    "{name} doesn't look like an image file.",
                ).format(name=Path(src).name),
                parent=self,
            )
            return
        try:
            ...
            show_error(
                tr("image_picker.import_failed_title", "Import failed"),
                tr(
                    "image_picker.import_failed_msg",
    ...
    def _show_help(self, _event=None) -> None:
        ...
        frame.pack()
        stk.Label(
            frame, text=tr("image_picker.help_text", HELP_TEXT), bg=BG, fg=HEADER_FG,
