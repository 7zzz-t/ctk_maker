...
class FontPickerDialog(ManagedToplevel):
    ...
    def __init__(
        ...
        self.window_title = tr("font.title", "Select font")
    ...
    def _build_header(self, parent) -> None:
        ...
        style.primary_button(
            bar, tr("font.import_file", "+ Import file..."),
            command=self._on_import, width=140,
        ).pack(side="left", padx=(10, 4), pady=10)
        style.secondary_button(
            bar, tr("font.add_system_font", "+ Add system font..."),
    ...
    def _build_preview(self, parent) -> None:
        ...
        wrap = stk.Frame(parent, bg=PANEL_BG, height=110)
        wrap.pack(side="top", fill="x", padx=8, pady=(8, 0))
        wrap.pack_propagate(False)

        head = stk.Frame(wrap, bg=PANEL_BG)
        head.pack(fill="x", padx=6, pady=(4, 2))
        stk.Label(
            head, text=tr("font.preview", "Preview"), bg=PANEL_BG, fg=HEADER_FG,
            font=ui_font(9, "bold"),
        ).pack(side="left")
        entry = stk.Entry(
        ...
        self._preview_body = stk.Frame(wrap, bg=PANEL_BG)
    ...
    def _build_footer(self, parent) -> None:
        ...
        scope_wrap = stk.Frame(parent, bg=BG)
        stk.Label(
            scope_wrap, text=tr("font.apply_to", "Apply to:"), bg=BG, fg=HEADER_FG,
            font=ui_font(10, "bold"), anchor="w",
        ).pack(fill="x", pady=(0, 4))
        scope_labels = (
            tr("font.scope_widget", "Just this widget"),
            tr("font.scope_type", "All {type}s").format(type=self.type_display),
            tr("font.scope_all", "Whole project"),
        )
        scope_to_value = {
        ...
        # action's importance.
        style.secondary_button(
            foot, tr("font.reset", "Reset"), command=self._on_use_default, width=70,
        ).grid(row=0, column=0, sticky="w")
        style.secondary_button(
            foot, tr("font.cancel", "Cancel"), command=self._on_cancel, width=90,
        ).grid(row=0, column=2, padx=(0, 8))
        self._ok_btn = style.primary_button(
            foot, tr("font.apply", "Apply"), command=self._on_ok, width=140,
        )
    ...
    def _on_row_right_click(self, event, family: str) -> None:
        ...
        menu.add_command(
            label=tr("font.remove_from_project", "Remove '{family}' from project...").format(family=family),
    ...
    def _remove_palette_family(self, family: str) -> None:
        ...
        if not ask_yes_no(
            tr("font.remove_title", "Remove from project"),
            tr("font.remove_message",
               "Remove '{family}' from this project?\n\n"
               "{file_detail}\n"
               ...
               "this family fall back to a default at next render.",
                ...
                family=family,
                file_detail=(
                    tr("font.remove_file_detail",
                       "File: {file}\n\nThe .ttf/.otf is deleted from disk."
                       ).format(file=file_path)
                    if file_path else
                    tr("font.remove_ref_only", "Reference only — no file to delete.\n")
                ),
            ),
            parent=self,
            danger=True,
            yes_text=tr("font.remove_yes", "Remove"),
            no_text=tr("font.cancel", "Cancel"),
        ):
            return
        ...
        if file_path and file_path.exists():
            try:
                ...
                show_error(
                    tr("font.remove_failed_title", "Remove failed"),
                    tr("font.remove_failed_msg", "Couldn't delete:\n{file}").format(file=file_path),
                    parent=self,
                )
                return
    ...
    def _on_import(self) -> None:
        if not self.project_file:
            show_info(
                tr("font.save_first_title", "Save first"),
                tr("font.save_first_msg",
        ...
        src = filedialog.askopenfilename(
            parent=self, title=tr("font.import_title", "Import font into project"),
            filetypes=[
                (tr("font.filetype_fonts", "Font files"), "*.ttf *.otf *.ttc"),
                (tr("font.filetype_all", "All files"), "*.*"),
            ],
        )
        if not src:
            return
        if Path(src).suffix.lower() not in FONT_EXTS:
            show_warning(
                tr("font.not_font_title", "Not a font"),
                tr("font.not_font_msg", "{name} doesn't look like a font file.").format(name=Path(src).name),
                parent=self,
            )
            return
        try:
            ...
            show_error(
                tr("font.import_failed_title", "Import failed"),
                tr("font.import_failed_msg",
                   "Could not copy the font into the project's assets folder."),
                parent=self,
            )
            return
        ...
        if family is None:
            show_warning(
                tr("font.no_family_title", "Font registered with no family"),
                tr("font.no_family_msg",
    ...
    def _show_help(self, _event=None) -> None:
        ...
        frame.pack()
        stk.Label(
            frame, text=tr("font.help_text", HELP_TEXT), bg=BG, fg=HEADER_FG,
...
class SystemFontPickerDialog(ManagedToplevel):
    ...
    def __init__(self, parent, exclude: set[str] | None = None):
        ...
        self._row_widgets: dict[str, dict] = {}
        self.window_title = tr("font.system_title", "Add system font")
    ...
    def _build_header(self, parent) -> None:
        ...
        bar.pack(fill="x")
        stk.Label(
            bar, text=tr("font.system_fonts", "System fonts"), bg=HEADER_BG, fg=HEADER_FG,
    ...
    def _build_footer(self, parent) -> None:
        ...
        self._ok_btn = style.primary_button(
            foot, tr("font.add", "Add"), command=self._on_ok, width=90,
        )
    ...
    def _refresh(self) -> None:
        ...
        if not families:
            ctk.CTkLabel(
                self._list_wrap,
                text=tr("font.no_matching", "No matching system fonts."),
                font=ui_font(10), text_color=DIM_FG,
            ).pack(pady=40)
            self._set_selected(None)
            return
