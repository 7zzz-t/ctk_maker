...
class ProjectPanelPages:
    ...
    def on_new_page(self) -> None:
        ...
        if not panel.project.folder_path:
            return
        name = ask_string(
            tr("pp.pages.new_page", "New page"),
            tr("pp.pages.page_name", "Page name:"),
            initialvalue=tr("pp.pages.new_page", "New page"),
        ...
        try:
            ...
            show_error(
                tr("pp.pages.new_page_failed", "New page failed"), str(exc),
                parent=panel.winfo_toplevel(),
            )
            return
    ...
    def on_context_duplicate_page(self) -> None:
        ...
        try:
            ...
            show_error(
                tr("pp.pages.duplicate_failed", "Duplicate failed"), str(exc),
                parent=panel.winfo_toplevel(),
            )
            return
        if panel.path_provider():
    ...
    def on_context_rename_page(self) -> None:
        ...
        try:
            ...
            show_error(
                tr("pp.pages.rename_failed", "Rename failed"), str(exc),
                parent=panel.winfo_toplevel(),
            )
            return
    ...
    def _do_delete_page(self) -> None:
        ...
        if len(panel.project.pages or []) <= 1:
            show_info(
                tr("pp.pages.cannot_delete_page", "Cannot delete page"),
                tr(
                    "pp.pages.cannot_delete_page_msg",
        ...
        try:
            ...
            show_error(
                tr("pp.pages.delete_failed", "Delete failed"), str(exc),
                parent=panel.winfo_toplevel(),
            )
            return
