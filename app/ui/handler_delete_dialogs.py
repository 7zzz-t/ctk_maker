...
class WindowDeleteDialog(ManagedToplevel):
    ...
    def __init__(
        ...
        self.window_title = tr("delete.window_title", "Delete window")
    ...
    def build_content(self) -> ctk.CTkFrame:
        ...
        ctk.CTkLabel(
            body,
            text=tr("delete.window_confirm", 'Delete "{name}"?').format(
        ...
        if self._widget_count == 1:
            widget_line = tr("delete.one_widget", "• 1 widget")
        else:
            widget_line = tr(
                "delete.n_widgets", "• {count} widgets",
            ).format(count=self._widget_count)
        if self._local_var_count == 1:
            var_line = tr(
                "delete.one_local_var", "• 1 local variable (will be lost)",
            )
        else:
            var_line = tr(
                "delete.n_local_vars",
                "• {count} local variables (will be lost)",
            ).format(count=self._local_var_count)
        bullets = [widget_line, var_line]
        ctk.CTkLabel(
            info, text="\n".join(bullets),
            font=ui_font(10),
            text_color=_BODY_FG,
        ...
        ctk.CTkButton(
            footer, text=tr("delete.window_button", "Delete window"),
            width=140, height=34, corner_radius=4,
            fg_color=_DANGER_BG, hover_color=_DANGER_HOVER,
            command=self._on_delete,
        ).pack(side="right")
        ctk.CTkButton(
            footer, text=tr("delete.cancel", "Cancel"),
...
class PageDeleteDialog(ManagedToplevel):
    ...
    def __init__(self, parent, page_name: str):
        self.window_title = tr("delete.page_title", "Delete page")
        self.confirmed: bool = False
    ...
    def build_content(self) -> ctk.CTkFrame:
        ...
        ctk.CTkLabel(
            body,
            text=tr("delete.page_confirm", 'Delete "{name}"?').format(
        ...
        footer.pack(fill="x", padx=22, pady=(16, 16))
        ctk.CTkButton(
            footer, text=tr("delete.page_button", "Delete page"),
