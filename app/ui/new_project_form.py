...
def _label_for_size(w: int, h: int) -> tuple[str, str]:
    ...
    return tr("new_project_form.custom", "Custom"), "Custom"
...
class NewProjectForm(ctk.CTkFrame):
    ...
    def _build(self) -> None:
        ctk.CTkLabel(
            self, text=tr("new_project_form.title", "New Project"),
            font=ui_font(11, "bold"),
            text_color=SUBTITLE_FG, anchor="w",
        ).pack(fill="x", padx=14, pady=(10, 10))

        self._add_row(tr("new_project_form.name", "Name"), self._build_name_entry)
        self._add_row(tr("new_project_form.save_to", "Save to"), self._build_save_dir_row)
        self._build_preview_label()
        self._refresh_preview()
        self._add_separator()
        self._add_row(tr("new_project_form.device", "Device"), self._build_device_dropdown)
        self._add_row(tr("new_project_form.screen_size", "Screen Size"), self._build_screen_dropdown)
        self._add_separator()
        self._add_row(tr("new_project_form.width", "Width"), self._build_width_entry)
        self._add_row(tr("new_project_form.height", "Height"), self._build_height_entry)
    ...
    def _on_device_selected(self, device: str) -> None:
        ...
        if not self._size_map:
            custom_label = tr("new_project_form.custom", "Custom")
            self._screen_menu.configure(values=[custom_label], state="disabled")
            self._screen_var.set(custom_label)
            return
    ...
    def _on_pick_save_dir(self) -> None:
        path = filedialog.askdirectory(
            ...
            title=tr("new_project_form.choose_save_location", "Choose save location"),
    ...
    def validate_and_get(self) -> tuple[str, str, int, int] | None:
        ...
        if any(c in FORBIDDEN_NAME_CHARS for c in name):
            self._flag_name_error()
            show_warning(
                tr("new_project_form.invalid_name_title", "Invalid name"),
                tr("new_project_form.invalid_name_msg",
        ...
        if target_folder.exists():
            self._flag_name_error()
            show_warning(
                tr("new_project_form.folder_exists_title", "Folder exists"),
                tr("new_project_form.folder_exists_msg",
                   "A folder named '{name}' already exists at:\n\n"
                   "{path}\n\n"
                   "Pick a different project name or save location.").format(
                    name=name, path=save_dir,
                ),
                parent=self.winfo_toplevel(),
            )
            return None
        ...
        try:
            ...
            self._flag_name_error()
            show_error(
                tr("new_project_form.save_unwritable_title", "Save location unwritable"),
                tr("new_project_form.save_unwritable_msg",
                   "Could not create:\n\n{path}\n\n"
                   "Pick a different save location.").format(path=target_folder),
                parent=self.winfo_toplevel(),
            )
            return None
