...
class MenuMixin(_MainWindowHost):
    ...
    def _rebuild_windows_menu(self) -> None:
        ...
        for doc in self.project.documents:
            label = doc.name or tr("menu.doc_untitled", "Untitled")
            if doc.is_toplevel:
                label = tr("menu.doc_dialog_suffix", "{label}  (Dialog)").format(
                    label=label,
                )
    ...
    def _build_menubar(self) -> None:
        ...
        self._add_cmd(file_menu, tr("menu.file.new", "New..."), self._on_new, icon="file-plus", accelerator=f"{MOD_LABEL_PLUS}N")
        self._add_cmd(file_menu, tr("menu.file.open", "Open..."), self._on_open, icon="folder-open", accelerator=f"{MOD_LABEL_PLUS}O")
        self._add_cmd(
            file_menu, tr("menu.file.recover_backup", "Recover from Backup..."),
            self._on_recover_from_backup, icon="rotate-ccw",
        )

        self._recent_menu = tk.Menu(file_menu, tearoff=0, **menu_style())
        self._add_cascade(file_menu, tr("menu.file.recent_projects", "Recent Projects"), self._recent_menu, icon="history")
        self._rebuild_recent_menu()

        file_menu.add_separator()
        self._add_cmd(file_menu, tr("menu.file.save", "Save"), self._on_save, icon="save", accelerator=f"{MOD_LABEL_PLUS}S")
        self._add_cmd(file_menu, tr("menu.file.save_as", "Save As..."), self._on_save_as, icon="save-all", accelerator=f"{MOD_LABEL_PLUS}Shift+S")
        file_menu.add_separator()
        self._add_cmd(file_menu, tr("menu.file.export_python", "Export to Python..."), self._on_export, icon="file-code")
        self._add_cmd(
            file_menu, tr("menu.file.export_active", "Export Active Document..."),
            self._on_export_active_document, icon="file-code",
        )
        self._add_cmd(
            file_menu, tr("menu.file.run_script", "Run Python Script..."),
            self._on_run_script, icon="tv-minimal-play",
        )
        file_menu.add_separator()
        self._add_cmd(file_menu, tr("menu.file.quit", "Quit"), self._on_quit, icon="log-out", accelerator=f"{MOD_LABEL_PLUS}Q")
        menubar.add_cascade(label=tr("menu.file", "File"), menu=file_menu)
        ...
        self._add_cmd(
            edit_menu, tr("menu.edit.undo", "Undo"), self._on_undo, accelerator=f"{MOD_LABEL_PLUS}Z",
        )
        self._add_cmd(
            edit_menu, tr("menu.edit.redo", "Redo"), self._on_redo, accelerator=f"{MOD_LABEL_PLUS}Y",
        )
        edit_menu.add_separator()
        self._add_cmd(
            edit_menu, tr("menu.edit.cut", "Cut"), self._on_menu_cut,
            accelerator=f"{MOD_LABEL_PLUS}X",
        )
        self._add_cmd(
            edit_menu, tr("menu.edit.copy", "Copy"), self._on_menu_copy,
            accelerator=f"{MOD_LABEL_PLUS}C",
        )
        self._add_cmd(
            edit_menu, tr("menu.edit.paste", "Paste"), self._on_menu_paste,
            accelerator=f"{MOD_LABEL_PLUS}V",
        )
        self._add_cmd(
            edit_menu, tr("menu.edit.duplicate", "Duplicate"), self._on_menu_duplicate,
            accelerator=f"{MOD_LABEL_PLUS}D",
        )
        self._add_cmd(
            edit_menu, tr("menu.edit.rename", "Rename"), self._on_menu_rename,
            accelerator=f"{MOD_LABEL_PLUS}I",
        )
        self._add_cmd(
            edit_menu, tr("menu.edit.delete", "Delete"), self._on_menu_delete,
            accelerator="Del",
        )
        edit_menu.add_separator()
        self._add_cmd(
            edit_menu, tr("menu.edit.select_all", "Select All"), self._on_menu_select_all,
            accelerator=f"{MOD_LABEL_PLUS}A",
        )
        edit_menu.add_separator()
        self._add_cmd(
            edit_menu, tr("menu.edit.group", "Group"), self._on_group_shortcut,
            accelerator=f"{MOD_LABEL_PLUS}G",
        )
        self._add_cmd(
            edit_menu, tr("menu.edit.ungroup", "Ungroup"), self._on_ungroup_shortcut,
        ...
        edit_menu.add_separator()
        self._add_cmd(
            edit_menu, tr("menu.edit.bring_front", "Bring to Front"), self._on_menu_bring_to_front,
        )
        self._add_cmd(
            edit_menu, tr("menu.edit.send_back", "Send to Back"), self._on_menu_send_to_back,
        )
        edit_menu.add_separator()
        self._add_cmd(
            edit_menu, tr("menu.edit.export_component", "Export Component…"), self._on_menu_export_component,
        )
        menubar.add_cascade(label=tr("menu.edit", "Edit"), menu=edit_menu)
        ...
        self._add_cmd(form_menu, tr("menu.window.preview", "Preview"), self._on_preview,
                      icon="play", accelerator=f"{MOD_LABEL_PLUS}R / F5")
        self._add_cmd(form_menu, tr("menu.window.preview_active", "Preview Active Dialog"), self._on_preview_active,
                      icon="square-play", accelerator=f"{MOD_LABEL_PLUS}P")
        form_menu.add_separator()
        self._add_cmd(form_menu, tr("menu.window.add_dialog", "Add Dialog"), self._on_add_dialog,
                      icon="plus", accelerator=f"{MOD_LABEL_PLUS}M")
        self._add_cmd(form_menu, tr("menu.window.remove", "Remove"), self._on_remove_current_document,
                      icon="trash-2")
        form_menu.add_separator()
        self._add_cmd(form_menu, tr("menu.window.rename", "Rename"), self._on_rename_current_doc,
                      icon="pencil", accelerator=f"{MOD_LABEL_PLUS}I")
        self._add_cmd(form_menu, tr("menu.window.settings", "Window Settings"), self._on_form_settings,
                      icon="settings")
        ...
        self._add_cmd(
            visibility_menu, tr("menu.window.minimize", "Minimize Window"),
            self._on_minimize_active_doc, icon="chevron-down",
        )
        self._add_cmd(
            visibility_menu, tr("menu.window.minimize_all", "Minimize All"),
            self._on_minimize_all_docs, icon="chevrons-down",
        )
        self._add_cmd(
            visibility_menu, tr("menu.window.restore_all", "Restore All"),
            self._on_restore_all_docs, icon="chevrons-up",
        )
        self._add_cascade(
            form_menu, tr("menu.window.visibility", "Visibility"), visibility_menu, icon="eye",
        )
        form_menu.add_separator()
        self._add_cmd(form_menu, tr("menu.window.move_up", "Move Up"), self._on_move_doc_up,
                      icon="arrow-up")
        self._add_cmd(form_menu, tr("menu.window.move_down", "Move Down"), self._on_move_doc_down,
                      icon="arrow-down")
        form_menu.add_separator()
        ...
        form_menu.add_cascade(label=tr("menu.window.all_windows", "All Windows"), menu=self._windows_menu)
        menubar.add_cascade(label=tr("menu.window", "Window"), menu=form_menu)
        ...
        view_menu.add_checkbutton(
            label=tr("menu.view.console", "Console"),
            variable=self._console_dock_var,
            command=self._on_toggle_console_dock,
            accelerator="F12",
        )
        view_menu.add_checkbutton(
            label=tr("menu.view.console_floating", "Console (floating)"),
            variable=self._console_window_var,
            command=self._on_toggle_console_window,
        )
        menubar.add_cascade(label=tr("menu.view", "View"), menu=view_menu)

        # ---- Data ----
        data_menu = tk.Menu(menubar, tearoff=0, **menu_style())
        self._add_cmd(
            data_menu, tr("menu.data.global_vars", "Global Variables"),
            lambda: self._on_request_open_variables_window("global"),
        )
        self._add_cmd(
            data_menu, tr("menu.data.local_vars", "Local Variables"),
            lambda: self._on_request_open_variables_window("local"),
        )
        menubar.add_cascade(label=tr("menu.data", "Data"), menu=data_menu)

        # ---- Tools ----
        tools_menu = tk.Menu(menubar, tearoff=0, **menu_style())
        self._add_cmd(
            tools_menu, tr("menu.tools.inspect", "Inspect CTk Widget..."),
            self._on_inspect_widget, icon="search",
        )
        self._add_cmd(
            tools_menu, tr("menu.tools.transitions", "Transitions Demo"),
            self._on_open_transitions_demo, icon="sparkles",
        )
        self._add_cmd(
            tools_menu, tr("menu.tools.color_palette", "Color Palette"),
            self._on_open_color_palette, icon="palette",
        )
        menubar.add_cascade(label=tr("menu.tools", "Tools"), menu=tools_menu)

        # ---- Settings ----
        settings_menu = tk.Menu(menubar, tearoff=0, **menu_style())
        self._add_cmd(
            settings_menu, tr("menu.settings.preferences", "Preferences..."),
            self._on_open_preferences, icon="settings",
            accelerator=f"{MOD_LABEL_PLUS},",
        )
        menubar.add_cascade(label=tr("menu.settings", "Settings"), menu=settings_menu)

        # ---- Help ----
        help_menu = tk.Menu(menubar, tearoff=0, **menu_style())
        self._add_cmd(
            help_menu, tr("menu.help.documentation", "Documentation"), self._on_widget_docs,
            icon="book-open", accelerator=f"{MOD_LABEL_PLUS}Shift+I",
        )
        help_menu.add_separator()
        self._add_cmd(
            help_menu, tr("menu.help.user_guide", "User Guide"), self._on_user_guide,
            icon="book-open",
        )
        self._add_cmd(
            help_menu, tr("menu.help.widgets", "Widgets"), self._on_widget_catalog,
            icon="layout-list",
        )
        self._add_cmd(
            help_menu, tr("menu.help.shortcuts", "Keyboard Shortcuts"), self._on_keyboard_shortcuts,
            icon="square-arrow-out-up-right",
        )
        help_menu.add_separator()
        self._add_cmd(
            help_menu, tr("menu.help.report_bug", "Report a Bug"), self._on_report_bug,
            icon="bug-play",
        )
        help_menu.add_separator()
        self._add_cmd(help_menu, tr("menu.help.about", "About..."), self._on_about, icon="info")
        menubar.add_cascade(label=tr("menu.help", "Help"), menu=help_menu)
    ...
    def _rebuild_recent_menu(self) -> None:
        ...
        if not paths:
            self._recent_menu.add_command(label=tr("menu.file.recent_empty", "(empty)"), state="disabled")
        else:
            for p in paths:
        ...
        self._recent_menu.add_command(
            label=tr("menu.file.clear_menu", "Clear Menu"), command=self._on_clear_recent,
        )
    ...
    def _refresh_edit_menu_state(self) -> None:
        ...
        states = {
            tr("menu.edit.undo", "Undo"): self.project.history.can_undo(),
            tr("menu.edit.redo", "Redo"): self.project.history.can_redo(),
            tr("menu.edit.copy", "Copy"): has_selection,
            tr("menu.edit.paste", "Paste"): has_clipboard,
            tr("menu.edit.delete", "Delete"): has_selection,
            tr("menu.edit.select_all", "Select All"): has_any,
            tr("menu.edit.group", "Group"): can_group,
            tr("menu.edit.ungroup", "Ungroup"): can_ungroup,
            tr("menu.edit.bring_front", "Bring to Front"): has_selection,
            tr("menu.edit.send_back", "Send to Back"): has_selection,
        }
        try:
            last = menu.index("end")
        except tk.TclError:
    ...
    def _on_menu_delete(self) -> None:
        ...
        confirmed = ask_yes_no(
            title=tr("dialog.delete_widget_title", "Delete widget"),
            message=tr("dialog.delete_widget_msg", "Delete this {type}?").format(type=type_label),
            parent=self,
            danger=True,
            yes_text=tr("dialog.delete", "Delete"),
            no_text=tr("dialog.cancel", "Cancel"),
        )
        if not confirmed:
            return
        snapshot = node.to_dict()
    ...
    def _build_align_submenu(self, parent_menu) -> None:
        ...
        _add(tr("menu.edit.align_left", "Align Left"), MODE_LEFT)
        _add(tr("menu.edit.align_center_h", "Align Center Horizontally"), MODE_CENTER_H)
        _add(tr("menu.edit.align_right", "Align Right"), MODE_RIGHT)
        align_menu.add_separator()
        _add(tr("menu.edit.align_top", "Align Top"), MODE_TOP)
        _add(tr("menu.edit.align_middle_v", "Align Middle Vertically"), MODE_CENTER_V)
        _add(tr("menu.edit.align_bottom", "Align Bottom"), MODE_BOTTOM)
        align_menu.add_separator()
        _add(tr("menu.edit.distribute_h", "Distribute Horizontally"), MODE_DISTRIBUTE_H)
        _add(tr("menu.edit.distribute_v", "Distribute Vertically"), MODE_DISTRIBUTE_V)
        parent_menu.add_cascade(label=tr("menu.edit.align", "Align"), menu=align_menu)
    ...
    def _on_menu_export_component(self) -> None:
        ...
        if path is None:
            show_info(
                tr("dialog.pick_component_title", "Pick a component"),
                tr("dialog.pick_component_msg",
    ...
    def _on_reset_advisories(self) -> None:
        ...
        if not advisory_keys:
            show_info(
                tr("warnings.reset_title", "Warnings reset"),
                tr("warnings.reset_none", "No dismissed warnings to reset."),
                parent=self,
            )
            return
        ...
        show_info(
            tr("warnings.reset_title", "Warnings reset"),
            tr(
                "warnings.reset_cleared",
                "Cleared {count} dismissed warning(s). "
                "They'll surface again on their next trigger.",
            ).format(count=len(advisory_keys)),
            parent=self,
        )
