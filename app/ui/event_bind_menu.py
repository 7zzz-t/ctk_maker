...
def populate_event_bind_menu(
    ...
    **Scripts** group listing every CTkScript component attached to the
    ...
    if node is None:
        _add_disabled_hint(
            menu, tr("event_bind.no_widget_selected", "No widget selected"),
        )
        return
    if event_by_key(node.widget_type, event_key) is None:
        _add_disabled_hint(
            menu, tr("event_bind.unknown_event", "Unknown event"),
        )
        return
    ...
    if document is None:
        _add_disabled_hint(
            menu,
            tr("event_bind.open_project_to_bind",
               "Open the project to bind handlers"),
        )
        return
    ...
    if not comp_targets:
        _add_disabled_hint(
            menu,
            tr("event_bind.no_actions_hint",
               "No actions yet — attach a script (＋ in the Scripts group)"),
        )
        return
    scripts_menu = tk.Menu(menu, tearoff=0, **style)
    for script_rel, cls, scope in comp_targets:
        label_scope = (
            tr("event_bind.scope_this_widget", "this widget")
            if scope == "widget"
            else tr("event_bind.scope_window", "window")
        )
        ...
        if methods:
            ...
            _add_disabled_hint(
                sub, tr("event_bind.no_public_methods", "No public methods"),
            )
        scripts_menu.add_cascade(label=f"{cls}  ({label_scope})", menu=sub)
    menu.add_cascade(
        label=tr("event_bind.scripts", "Scripts"), menu=scripts_menu,
    )
