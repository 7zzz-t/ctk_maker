...
def _relative_time(ts: float) -> str:
    ...
    diff = time.time() - ts
    if diff < 60:
        return tr("recent.time_just_now", "just now")
    if diff < 3600:
        return tr("recent.time_minutes_ago", "{n}m ago").format(
            n=int(diff / 60),
        )
    if diff < 86400:
        return tr("recent.time_hours_ago", "{n}h ago").format(
            n=int(diff / 3600),
        )
    if diff < 604800:
        return tr("recent.time_days_ago", "{n}d ago").format(
            n=int(diff / 86400),
        )
    if diff < 2592000:
        return tr("recent.time_weeks_ago", "{n}w ago").format(
            n=int(diff / 604800),
        )
    if diff < 31536000:
        return tr("recent.time_months_ago", "{n}mo ago").format(
            n=int(diff / 2592000),
        )
    return tr("recent.time_years_ago", "{n}y ago").format(
        n=int(diff / 31536000),
    )
...
class RecentList(ctk.CTkFrame):
    def __init__(
        ...
        *,
        ...
        ctk.CTkLabel(
            self, text=tr("recent.title", "Recent"),
    ...
    def refresh(self) -> None:
        ...
        if not recent:
            ctk.CTkLabel(
                self._scroll,
                text=tr("recent.empty", "No recent projects"),
    ...
    def _build_row(self, path: str) -> None:
        ...
        try:
            ...
            time_text = tr("recent.missing", "missing")
            missing = True
    ...
    def _show_row_menu(self, event, path: str) -> str:
        ...
        menu.add_command(
            label=tr("recent.menu_open", "Open"),
            state="normal" if not missing else "disabled",
            command=lambda p=path: self._activate(p),
        )
        menu.add_command(
            label=tr("recent.menu_open_folder", "Open containing folder"),
            state="normal" if not missing else "disabled",
            command=lambda p=path: self._reveal_in_explorer(p),
        )
        menu.add_separator()
        menu.add_command(
            label=tr("recent.menu_remove", "Remove from Recent"),
