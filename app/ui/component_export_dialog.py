...
class ComponentExportDialog(ManagedToplevel):
    ...
    def __init__(self, parent, source_path: Path):
        self.window_title = tr("comp_export.title", "Export component")
    ...
    def build_content(self) -> ctk.CTkFrame:
        ...
        ctk.CTkLabel(
            body, text=tr("comp_export.name", "Name"), font=ui_font(10),
        ...
        ctk.CTkLabel(
            body, text=tr("comp_export.author", "Author"), font=ui_font(10),
        ).grid(row=4, column=0, sticky="w", pady=(0, 4))
        ctk.CTkEntry(
            body, textvariable=self._author_var, width=320,
            placeholder_text=tr("comp_export.optional", "optional"),
        ).grid(row=5, column=0, sticky="ew", pady=(0, 12))

        ctk.CTkLabel(
            body, text=tr("comp_export.destination_folder", "Destination folder"), font=ui_font(10),
        ...
        ctk.CTkEntry(
            ...
            placeholder_text=tr("comp_export.pick_folder", "(pick a folder)"), height=28,
        ).grid(row=0, column=0, sticky="ew", padx=(0, 6))
        ctk.CTkButton(
            path_row, text=tr("comp_export.browse", "Browse…"), width=80, height=28,
        ...
        ctk.CTkButton(
            footer, text=tr("comp_export.export", "Export"), width=120, height=32,
            corner_radius=4, command=self._on_export,
        ).pack(side="right")
        ctk.CTkButton(
            footer, text=tr("comp_export.cancel", "Cancel"), width=90, height=32,
            corner_radius=4,
    ...
    def _on_browse(self) -> None:
        path = filedialog.askdirectory(
            parent=self,
            title=tr("comp_export.browse_title", "Export component — pick destination folder"),
        )
        if not path:
            return
    ...
    def _on_export(self) -> None:
        ...
        if not dest:
            self.bell()
            show_warning(
                tr("comp_export.pick_dest_title", "Pick a destination"),
                tr("comp_export.pick_dest_msg", "Click Browse… to choose a destination folder."),
                parent=self,
            )
            return
        ...
        if not _is_valid_name(name):
            self.bell()
            show_warning(
                tr("comp_export.invalid_name_title", "Invalid name"),
                tr("comp_export.invalid_name_msg", "Names can't be empty or contain \\ / : * ? \" < > |."),
                parent=self,
            )
            return
        dest_dir = Path(dest)
        if not dest_dir.is_dir():
            self.bell()
            show_warning(
                tr("comp_export.folder_not_found_title", "Folder not found"),
                tr(
                    "comp_export.folder_not_found_msg",
        ...
        if dest_path.exists():
            overwrite = ask_yes_no(
                tr("comp_export.exists_title", "Already exists"),
                tr(
                    "comp_export.exists_msg",
        ...
        try:
            ...
            show_error(
                tr("comp_export.export_failed_title", "Export failed"),
                tr(
                    "comp_export.export_failed_msg",
                    "Couldn't write to:\n{path}\n\n{error}",
                ).format(path=dest_path, error=exc),
                parent=self,
            )
