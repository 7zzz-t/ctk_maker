...
PLACEHOLDER_SEVERITY = tr(
    "bug_report.placeholder_severity", "- Select severity -",
)
PLACEHOLDER_REPRO = tr("bug_report.placeholder_select", "- Select -")
PLACEHOLDER_AREA = tr("bug_report.placeholder_area", "- Select area -")
...
def attach_context_menu(widget) -> None:
    ...
    cut_label = tr("bug.menu.cut", "Cut")
    copy_label = tr("bug.menu.copy", "Copy")
    paste_label = tr("bug.menu.paste", "Paste")
    select_all_label = tr("bug.menu.select_all", "Select All")

    menu.add_command(
        label=cut_label, command=lambda: inner.event_generate("<<Cut>>"),
    )
    menu.add_command(
        label=copy_label, command=lambda: inner.event_generate("<<Copy>>"),
    )
    menu.add_command(
        label=paste_label, command=lambda: inner.event_generate("<<Paste>>"),
    )
    menu.add_separator()
    menu.add_command(label=select_all_label, command=select_all)

    def show(event):
        menu.entryconfig(
            cut_label, state="normal" if has_selection() else "disabled",
        )
        menu.entryconfig(
            copy_label, state="normal" if has_selection() else "disabled",
        )
        menu.entryconfig(
            paste_label, state="normal" if has_clipboard() else "disabled",
        )
        try:
...
def build_screenshot_hint_panel(
    ...
    ctk.CTkLabel(
        text_col, text=tr("bug.screenshot.heading", "Attaching a screenshot"),
    ...
    ctk.CTkLabel(
        text_col,
        text=tr(
            "bug.screenshot.github",
            "GitHub: drop or paste it on the page after it opens.",
        ),
...
class BugReporterWindow(ManagedToplevel):
    ...
    def __init__(self, master) -> None:
        self.window_title = tr("bug.window_title", "Report a Bug — CTkMaker")
    ...
    def _build_picker(self, parent) -> ctk.CTkFrame:
        ...
        ctk.CTkLabel(
            f, text=tr("bug.picker.heading", "What would you like to report?"),
            font=ui_font(18, "bold"),
            text_color=TITLE_FG,
        ).pack(pady=(0, 6))
        ctk.CTkLabel(
            f,
            text=tr("bug.picker.subtitle", "Pick a track to start filling out the report."),
            font=ui_font(11),
            text_color=DIM_FG,
        ).pack(pady=(0, 24))
        ...
        self._build_picker_card(
            ...
            icon_name="bug",
            icon_color="#e09c5c",
            title=tr("bug.picker.bug_title", "Bug Report"),
            title_color="#e09c5c",
            subtitle=tr(
                "bug.picker.bug_subtitle",
        ...
        self._build_picker_card(
            ...
            icon_name="lightbulb",
            icon_color="#5bc0f8",
            title=tr("bug.picker.feature_title", "Feature Request"),
            title_color="#5bc0f8",
            subtitle=tr(
                "bug.picker.feature_subtitle",
    ...
    def _build_intro_banner(self, parent) -> ctk.CTkFrame:
        ...
        ctk.CTkLabel(
            head_row, text=tr("bug.intro.why", "Why this matters"),
        ...
        guide_btn = ctk.CTkButton(
            action_row,
            text=tr("bug.intro.read_guide", "Read the full reporting guide  →"),
        ...
        issues_btn = ctk.CTkButton(
            action_row,
            text=tr("bug.intro.browse_issues", "Browse open issues first  →"),
    ...
    def _build_form(self, parent) -> None:
        ...
        ctk.CTkButton(
            topbar, text=tr("bug.form.change_mode", "← Change mode"),
        ...
        ctk.CTkLabel(
            title_frame, text=tr("bug.form.title_label", "Title *"),
            font=ui_font(11),
        ...
        title_entry = ctk.CTkEntry(
            ...
            placeholder_text=tr("bug.form.title_placeholder", "Short summary"),
        ...
        self._report_btn = _make_button(
            btns, text=tr("bug.form.report", "Report"), command=self._on_report,
    ...
    def _build_bug_form(self, parent) -> ctk.CTkFrame:
        ...
        self._mini_label(col_s, tr("bug.bug_form.severity", "Severity"))
        ...
        self._mini_label(col_r, tr("bug.bug_form.reproducibility", "Reproducibility"))
        ...
        self._mini_label(col_a, tr("bug.bug_form.area", "Area"))
        self._mini_optionmenu(col_a, self._area_var, AREA_VALUES)

        ctk.CTkLabel(
            f, text=tr("bug.bug_form.environment", "Environment"),
            font=ui_font(11, "bold"),
            text_color=TITLE_FG, anchor="w",
        ).pack(fill="x", pady=(8, 4))
        ...
        self._mini_label(col_v, tr("bug.bug_form.version", "CTkMaker version"))
        ...
        self._mini_label(col_o, tr("bug.bug_form.os", "OS"))
        ...
        self._mini_label(col_p, tr("bug.bug_form.python_version", "Python version"))
        ...
        self._desc_box, self._desc_counter = (
            self._labeled_textbox_with_counter(
                f, tr("bug.bug_form.description", "Description"),
                min_words=MIN_DESCRIPTION_WORDS, height=70,
            )
        )
        self._steps_box = self._labeled_textbox(
            f, tr("bug.bug_form.steps", "Steps to reproduce"), height=70,
        )
        self._expected_box = self._labeled_textbox(
            f, tr("bug.bug_form.expected", "Expected vs Actual"), height=70,
        )
    ...
    def _build_feature_form(self, parent) -> ctk.CTkFrame:
        ...
        self._problem_box, self._problem_counter = (
            self._labeled_textbox_with_counter(
                f, tr("bug.feature_form.problem", "Problem it solves"),
                min_words=MIN_DESCRIPTION_WORDS, height=80,
            )
        )
        self._solution_box = self._labeled_textbox(
            f, tr("bug.feature_form.solution", "Proposed solution"), height=100,
        )
        self._alternatives_box = self._labeled_textbox(
            f, tr("bug.feature_form.alternatives", "Alternatives considered"), height=80,
        )
        build_screenshot_hint_panel(f, icon_size=22).pack(
            fill="x", pady=(10, 4),
        )
    ...
    def _labeled_textbox_with_counter(
        ...
        counter = ctk.CTkLabel(
            row,
            text=tr("bug.counter", "{words} / {min} words").format(
                words=0, min=min_words,
            ),
            font=ui_font(10),
    ...
    def _switch_mode(self, mode: str) -> None:
        ...
        if hasattr(self, "_form_heading"):
            heading_key = (
                "bug.picker.bug_title" if mode == "Bug Report"
                else "bug.picker.feature_title"
            )
            self._form_heading.configure(text=tr(heading_key, mode))
        self._refresh_all()
    ...
    def _on_autodetect(self) -> None:
        ...
        if env:
            ...
            self._env_preview.configure(
                text=tr("bug.auto_detected", "Auto-detected: {lines}").format(
                    lines=lines,
                ),
            )
        else:
            self._env_preview.configure(
                text=tr("bug.auto_detect_none", "Auto-detect produced no extra info."),
            )
        self._refresh_all()
    ...
    def _on_open(self) -> None:
        ...
        webbrowser.open(url)
        self._status_label.configure(text=tr("bug.opened_browser", "Opened in browser."))
    ...
    def _on_export(self) -> None:
        ...
        path = filedialog.asksaveasfilename(
            parent=self,
            title=tr("bug.export_title", "Export report"),
            defaultextension=".md",
            initialfile=suggested,
            filetypes=[
                (tr("bug.filetype_markdown", "Markdown"), "*.md"),
                (tr("bug.filetype_text", "Text"), "*.txt"),
                (tr("bug.filetype_all", "All files"), "*.*"),
            ],
        )
        if not path:
            return
        ...
        content = (
            f"# {title}\n\n"
            f"{tr('bug.send_to', 'Send this report to:')} {BUG_REPORT_EMAIL}\n\n"
            f"---\n\n"
        ...
        try:
            ...
            self._status_label.configure(
                text=tr("bug.export_failed", "Export failed: {exc}").format(exc=exc),
            )
            return
        self._status_label.configure(
            text=tr("bug.exported_to", "Exported to: {path}").format(path=path),
        )
    ...
    def _refresh_word_counters(self) -> None:
        for box, counter in (
            ...
            counter.configure(
                text=tr(
                    "bug.counter", "{words} / {min} words",
                ).format(words=words, min=MIN_DESCRIPTION_WORDS),
            )
    ...
    def _refresh_url_stats(self) -> None:
        ...
        url = self._current_url()
        text = tr(
            "bug.url_stats", "Body: {body} chars  |  URL: {url} chars",
        ).format(body=len(body), url=len(url))
        if len(url) > URL_PREFILL_LIMIT:
            color = ACCENT_ERR
            text += tr(
                "bug.url_over_limit",
                "  (over GitHub pre-fill limit of {limit})",
            ).format(limit=URL_PREFILL_LIMIT)
...
class ReportDialog(ManagedToplevel):
    ...
    def __init__(self, master: "BugReporterWindow") -> None:
        self.window_title = tr("bug.report_dialog.title", "Report")
        self._owner = master
        super().__init__(master)
    ...
    def build_content(self) -> ctk.CTkFrame:
        ...
        ctk.CTkLabel(
            container,
            text=tr(
                "bug.report_dialog.heading",
                "How would you like to send this report?",
            ),
            font=ui_font(13, "bold"),
            text_color=TITLE_FG,
        ).pack(pady=(16, 6))
        ...
        self._add_card(
            container,
            title=tr("bug.report_dialog.github_title", "On GitHub"),
            description=tr(
                "bug.report_dialog.github_desc",
                "Open the pre-filled report on GitHub to review and "
                "submit. GitHub account required.",
            ),
            actions=[
                (tr("bug.report_dialog.copy_url", "Copy URL"), self._do_copy),
                (tr("bug.report_dialog.open_browser", "Open in Browser"), self._do_github),
            ],
            link_text=tr("bug.report_dialog.see_known", "See known issues first →"),
            link_command=lambda: webbrowser.open(KNOWN_ISSUES_URL),
        )
        self._add_card(
            container,
            title=tr("bug.report_dialog.email_title", "By Email"),
            description=tr(
                "bug.report_dialog.email_desc",
                "Save the report as a file and attach it to an "
                "email. No GitHub account needed.",
            ),
            actions=[(tr("bug.report_dialog.save_file", "Save as File..."), self._do_email)],
        ...
        _make_button(
            cancel_row, text=tr("bug.cancel", "Cancel"), command=self.destroy,
            width=90,
        ).pack(side="right")
        return container
    ...
    def _add_card(
        ...
        *,
        ...
        if email:
            ...
            ctk.CTkLabel(
                email_row, text=tr("bug.report_dialog.send_to", "Send to:"),
                font=ui_font(10),
                text_color=DIM_FG,
            ...
            def _copy_email():
                try:
                    ...
                    copy_btn.configure(text=tr("bug.copied", "Copied"))
                    self.after(
                        1500,
                        lambda: copy_btn.configure(text=tr("bug.copy", "Copy")),
                    )
                except Exception:
                    pass

            copy_btn = ctk.CTkButton(
                email_row, text=tr("bug.copy", "Copy"), command=_copy_email,
    ...
    def _do_copy(self) -> None:
        ...
        self._owner._status_label.configure(
            text=tr(
                "bug.copied_chars", "Copied {n} chars to clipboard.",
            ).format(n=len(url)),
        )
