...
class ProjectPanelTree:
    ...
    def refresh_info_panel(self) -> None:
        ...
        lines = [
            file_path.name,
            tr("pp.tree.size", "Size: {}").format(_human_size(size_bytes)),
        ]
        if kind == "images":
            dims = _read_image_size(file_path)
            if dims is not None:
                lines.append(
                    tr("pp.tree.dimensions", "Dimensions: {} × {} px")
            ...
            if family:
                lines.append(
                    tr("pp.tree.family", "Family: {}").format(family),
                )
            lines.append(
                tr("pp.tree.format", "Format: {}").format(
