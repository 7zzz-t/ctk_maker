...
class PreviewMixin(_MainWindowHost):
    ...
    def _on_preview(self) -> None:
        ...
        if not self.project.root_widgets:
            show_info(
                tr("preview.title", "Preview"),
                tr("preview.empty_workspace", "Nothing to preview — workspace is empty."),
                parent=self,
            )
            return
        ...
        try:
            ...
            show_error(
                tr("preview.failed_title", "Preview failed"),
                tr("preview.generate_failed", "Could not generate preview file."),
                parent=self,
            )
            return
        ...
        try:
            ...
            show_error(
                tr("preview.failed_title", "Preview failed"),
                tr("preview.launch_failed", "Could not launch Python."),
                parent=self,
            )
            return
    ...
    def _on_run_script(self) -> None:
        ...
        path = filedialog.askopenfilename(
            parent=self,
            title=tr("preview.run_script_title", "Run a Python script"),
            initialdir=last_dir,
            filetypes=[
                (tr("preview.filetype_python", "Python"), "*.py"),
                (tr("preview.filetype_all", "All files"), "*.*"),
            ],
        )
        if not path:
            return
        ...
        if Path(path).suffix.lower() not in {".py", ".pyw"}:
            show_error(
                tr("preview.not_python_title", "Not a Python script"),
                tr(
                    "preview.not_python_msg",
                    "Run Python Script only accepts .py / .pyw files.\n\n"
                    "You picked:\n{path}",
                ).format(path=path),
                parent=self,
            )
        ...
        try:
            ...
            show_error(
                tr("preview.run_failed_title", "Run failed"),
                tr("preview.run_failed_msg", "Could not launch:\n{path}").format(
                    path=path,
                ),
                parent=self,
            )
    ...
    def _confirm_save_before_export(self) -> bool:
        ...
        if not self._dirty:
            return True
        show_warning(
            tr("preview.unsaved_title", "Unsaved changes"),
            tr("preview.unsaved_msg", "Please save your progress before exporting."),
            parent=self,
        )
        return False
    ...
    def _on_export_active_document(
        ...
        if self.project.folder_path:
            ...
            show_info(
                tr("preview.quick_export_title", "Quick export"),
                tr("preview.save_before_export", "Save the project before exporting."),
                parent=self,
            )
            return
        ...
        dlg = QuickExportDialog(
            self, doc.name or tr("preview.untitled", "Untitled"), preview_path,
        )
        ...
        try:
            ...
            show_error(
                tr("preview.export_failed_title", "Export failed"),
                tr(
                    "preview.export_folder_failed",
                    "Could not create exports folder:\n{path}",
                ).format(path=out_dir),
                parent=self,
            )
            return
        ...
        try:
            ...
            show_error(
                tr("preview.export_failed_title", "Export failed"),
                tr(
                    "preview.export_write_failed",
                    "Could not write the export:\n{path}\n\n{exc}",
                ).format(path=target, exc=exc),
                parent=self,
            )
            return
        ...
        try:
            ...
            display = target.name
        self._show_toast(
            tr("preview.exported_toast", "Exported: {path}").format(path=display),
            duration_ms=2400,
        )
