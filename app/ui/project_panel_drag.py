...
class ProjectPanelDragDrop:
    ...
    def _show_drag_ghost(self, source_iids: list[str]) -> None:
        ...
        text = (
            tr("pp.drag.one_item", "{} item").format(count)
            if count == 1
            else tr("pp.drag.n_items", "{} items").format(count)
        )
        tk.Label(
    ...
    def _move_into(self, sources: list[Path], target_dir: Path) -> None:
        ...
        for src in sources:
            ...
            if dst.exists():
                show_warning(
                    tr("pp.drag.already_exists", "Already exists"),
                    tr(
                        "pp.drag.exists",
                        "'{name}' already exists in '{dst}'. Skipping.",
                    ).format(name=src.name, dst=target_dir.name),
            ...
            try:
                ...
                show_error(
                    tr("pp.drag.move_failed", "Move failed"),
                    tr("pp.drag.move_failed_msg",
                       "Couldn't move:\n{src}\n→\n{dst}").format(
                        src=src, dst=dst,
                    ),
                    parent=panel.winfo_toplevel(),
