...
class ComponentsPanel(ctk.CTkFrame):
    ...
    def _build_header(self) -> None:
        ...
        self._add_btn = ctk.CTkButton(
            type_bar, text=tr("components_panel.library_actions", "Library actions"),
            image=plus_icon,
            compound="left",
    ...
    def _build_filter(self) -> None:
        ...
        self._filter_entry = ctk.CTkEntry(
            ...
            placeholder_text=tr("components_panel.search", "Search components…"),
            height=24,
            fg_color=FILTER_BG,
    ...
    def _format_type(self, meta: dict) -> str:
        ...
        if comp_type == "fragment" and len(node_types) == 1:
            ...
            return _TYPE_INITIALS.get(
                t, tr("components_panel.type_frm", "Frm") if t == "CTkFrame" else t[:3],
            )
        if comp_type == "fragment":
            return tr("components_panel.type_frag", "Frag")
        if comp_type == "window":
            return tr("components_panel.type_window", "Window")
    ...
    def _on_show_add_menu(self) -> None:
        ...
        self._menu_command(
            menu, tr("components_panel.new_folder", "New Folder..."), "folder",
            lambda r=root: self._on_new_folder(r),
        )
        self._menu_command(
            menu, tr("components_panel.import_component", "Import component..."), "file-plus",
            self._on_import,
        )
        menu.add_separator()
        self._menu_command(
            menu, tr("components_panel.show_library_explorer", "Show library folder in Explorer"), "folder-open",
    ...
    def _show_context_menu(
        ...
        if meta is None:
            if root is not None:
                ...
                menu.add_separator()
                self._menu_command(
                    menu, tr("components_panel.show_library_explorer", "Show library folder in Explorer"),
            ...
            if kind == "folder":
                self._menu_command(
                    menu, tr("components_panel.new_subfolder", "New Subfolder..."), "folder",
                    lambda p=entry_path: self._on_new_folder(p),
                )
                menu.add_separator()
                self._menu_command(
                    menu, tr("components_panel.show_in_explorer", "Show in Explorer"), "folder-open",
                    lambda p=entry_path: self._reveal(p),
                )
                self._menu_command(
                    menu, tr("components_panel.rename", "Rename..."), "pencil",
                    lambda p=entry_path: self._on_rename(p),
                )
                self._menu_command(
                    menu, tr("components_panel.delete_folder", "Delete folder..."), "trash-2",
                ...
                if meta.get("is_window"):
                    self._menu_command(
                        menu, tr("components_panel.insert_new_dialog", "Insert as new Dialog"), "app-window",
                        lambda p=entry_path: self._request_insert(p),
                    )
                else:
                    self._menu_command(
                        menu, tr("components_panel.insert_current_window", "Insert into current window"), "frame",
                        lambda p=entry_path: self._request_insert(p),
                    )
                self._menu_command(
                    menu, tr("components_panel.preview", "Preview"), "eye",
                    lambda p=entry_path: self._on_preview(p),
                )
                menu.add_separator()
                self._menu_command(
                    menu, tr("components_panel.export", "Export..."), "save",
                ...
                self._menu_command(
                    menu, tr("components_panel.delete_menu", "Delete..."), "trash-2",
    ...
    def _on_new_folder(self, parent_dir: Path) -> None:
        name = ask_string(
            tr("components_panel.new_folder_title", "New folder"),
            tr("components_panel.folder_name_prompt", "Folder name:"),
            initialvalue=tr("components_panel.default_folder_name", "New Folder"),
        ...
        if not _is_valid_name(name):
            show_warning(
                tr("components_panel.invalid_name_title", "Invalid name"),
                tr("components_panel.invalid_folder_name_msg", "Folder names can't contain \\ / : * ? \" < > |."),
                parent=self.winfo_toplevel(),
            )
            return
        ...
        if target.exists():
            show_warning(
                tr("components_panel.already_exists_title", "Already exists"),
                tr(
                    "components_panel.folder_exists_msg",
                    "'{name}' already exists in '{folder}'.",
                ).format(name=name, folder=parent_dir.name),
                parent=self.winfo_toplevel(),
            )
            return
        try:
            ...
            show_error(
                tr("components_panel.new_folder_failed_title", "New folder failed"),
                tr(
                    "components_panel.new_folder_failed_msg",
    ...
    def _on_rename(self, path: Path) -> None:
        ...
        name = ask_string(
            tr("components_panel.rename_title", "Rename"),
            tr("components_panel.new_name_prompt", "New name:"),
            initialvalue=old,
            parent=self.winfo_toplevel(),
        )
        if not name:
        ...
        if not _is_valid_name(name):
            show_warning(
                tr("components_panel.invalid_name_title", "Invalid name"),
                tr("components_panel.invalid_name_msg", "Names can't contain \\ / : * ? \" < > |."),
                parent=self.winfo_toplevel(),
            )
            return
        if name == old:
        ...
        if target.exists():
            show_warning(
                tr("components_panel.already_exists_title", "Already exists"),
                tr(
                    "components_panel.name_exists_msg",
                    "'{name}' already exists.",
        ...
        try:
            ...
            show_error(
                tr("components_panel.rename_failed_title", "Rename failed"),
                tr(
                    "components_panel.rename_failed_msg",
                    "Couldn't rename:\n{source}\n→\n{target}",
                ).format(source=path, target=target),
                parent=self.winfo_toplevel(),
            )
            return
        self.refresh()
    ...
    def _on_delete_folder(self, path: Path) -> None:
        confirm = ask_yes_no(
            tr("components_panel.delete_folder_title", "Delete folder"),
            tr(
                "components_panel.delete_folder_msg",
                "Delete folder '{name}' and everything inside it?",
            ).format(name=path.name),
            parent=self.winfo_toplevel(),
            danger=True,
            yes_text=tr("components_panel.delete", "Delete"),
            no_text=tr("components_panel.cancel", "Cancel"),
        )
        if not confirm:
            return
        try:
            ...
            show_error(
                tr("components_panel.delete_failed_title", "Delete failed"),
                tr(
                    "components_panel.delete_folder_failed_msg",
    ...
    def _on_delete_file(self, path: Path) -> None:
        confirm = ask_yes_no(
            tr("components_panel.delete_component_title", "Delete component"),
            tr(
                "components_panel.delete_component_msg",
                "Delete component '{name}'?",
            ).format(name=component_display_stem(path)),
            parent=self.winfo_toplevel(),
            danger=True,
            yes_text=tr("components_panel.delete", "Delete"),
            no_text=tr("components_panel.cancel", "Cancel"),
        ...
        try:
            ...
            show_error(
                tr("components_panel.delete_failed_title", "Delete failed"),
                tr(
                    "components_panel.delete_component_failed_msg",
                    "Couldn't delete component:\n{path}",
                ).format(path=path),
                parent=self.winfo_toplevel(),
            )
            return
    ...
    def _on_preview(self, path: Path) -> None:
        ...
        if payload is None:
            show_error(
                tr("components_panel.preview_unavailable_title", "Preview unavailable"),
                tr(
                    "components_panel.preview_unavailable_msg",
                    "'{name}' isn't a readable component.",
                ).format(name=path.name),
                parent=self.winfo_toplevel(),
            )
            return
    ...
    def _on_import(self) -> None:
        ...
        if target_root is None:
            show_info(
                tr("components_panel.save_project_first_title", "Save project first"),
                tr(
                    "components_panel.save_project_first_msg",
        ...
        source = filedialog.askopenfilename(
            parent=self.winfo_toplevel(),
            title=tr("components_panel.import_dialog_title", "Import component"),
            filetypes=[
                (
                    tr("components_panel.ctkmaker_component_filetype", "CTkMaker component"),
                    f"*{COMPONENT_EXT} *{PUBLISH_COMPONENT_EXT}",
                ),
                (tr("components_panel.all_files_filetype", "All files"), "*.*"),
            ],
        )
        if not source:
            return
        ...
        if load_metadata(source_path) is None:
            show_error(
                tr("components_panel.invalid_component_title", "Invalid component"),
                tr(
                    "components_panel.invalid_component_msg",
                    "'{name}' isn't a readable .ctkcomp file.",
                ).format(name=source_path.name),
                parent=self.winfo_toplevel(),
            )
            return
    ...
    def _move_into(self, source: Path, target_dir: Path) -> None:
        ...
        if dst.exists():
            show_warning(
                tr("components_panel.already_exists_title", "Already exists"),
                tr(
                    "components_panel.move_exists_msg",
                    "'{name}' already exists in '{folder}'.",
                ).format(name=source.name, folder=target_dir.name),
                parent=self.winfo_toplevel(),
            )
            return
        try:
            ...
            show_error(
                tr("components_panel.move_failed_title", "Move failed"),
                tr(
                    "components_panel.move_failed_msg",
                    "Couldn't move:\n{source}\n→\n{target}",
                ).format(source=source, target=dst),
                parent=self.winfo_toplevel(),
            )
            return
        self.refresh()
    ...
    def _show_drag_ghost(self, name: str) -> None:
        ...
        frame.pack()
        tk.Label(
            frame, text=tr("components_panel.drag_ghost_prefix", "+ {name}").format(name=name),
            bg="#3b8ed0", fg="white",
