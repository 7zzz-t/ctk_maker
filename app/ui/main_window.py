...
PROJECT_FILE_TYPES = [
    (tr("main_window.file_type_project", "CTkMaker project"), "*.ctkproj"),
    (tr("main_window.file_type_all", "All files"), "*.*"),
]
...
def _spawn_preview(tmp_dir: Path, tmp_path: Path, cwd: str) -> subprocess.Popen:
    ...
    if mode == "inapp":
        ...
        runner_path = _write_inapp_runner(tmp_dir, tmp_path)
        return subprocess.Popen(
            [py, "-u", str(runner_path)], cwd=cwd,
...
# Inapp runner: wraps the preview so any ``logging.*`` call lands in
# the in-app console alongside ``print``. Without it, DEBUG/INFO are
# silently swallowed by Python's default ``logging`` config (the
# ``lastResort`` handler only forwards WARNING+ to stderr).
_INAPP_RUNNER_TEMPLATE = '''"""Inapp preview runner — installs logging + crash plumbing.

Generated by CTkMaker; do not edit. Wraps the preview entry point so:

- ``logging.basicConfig(level=DEBUG, force=True)`` routes every log
  record to stderr (which CTkMaker's in-app console captures), in a
  Unity-style ``LEVEL | name | message`` shape the console parses for
  per-level colouring.
- ``sys.excepthook`` writes uncaught tracebacks to stderr so they
  reach the console even if the preview crashes outside Tk.
- ``sys.argv[0]`` matches the user script path so anything reading
  ``sys.argv[0]`` for path-derived logic works as if launched directly.
- stdout/stderr are line-buffered in the child too (parent already
  pipes them, but the producer side needs to flush eagerly for
  per-line streaming through the pipe).

User code can call ``logging.basicConfig`` itself afterwards to
override format/level — that is intentional, the bootstrap is a
...
def _write_inapp_runner(tmp_dir: Path, tmp_path: Path) -> Path:
    """Drop ``inapp_runner.py`` next to ``preview.py``. The runner runs
    ``preview.py`` via ``runpy`` after installing logging defaults +
    crash hook so ``logging.*`` flows into the in-app console.
    """
    runner_path = tmp_dir / "inapp_runner.py"
    runner_path.write_text(
        _INAPP_RUNNER_TEMPLATE.format(user_script=str(tmp_path)),
        encoding="utf-8",
    )
    return runner_path
...
def _confirm_var_name_fallbacks(parent) -> bool:
    ...
    for doc_name, rows in by_doc.items():
        ...
        if rest > 0:
            lines.append(
                "      "
                + tr("main_window.fallbacks_more", "… (+{n} more)").format(
                    n=rest,
                ),
            )
    lines.append("")
    ...
    lines.append(
        tr(
            "main_window.fallbacks_continue",
            "Continue with the preview anyway?",
        ),
    )
    return ask_yes_no(
        tr("main_window.fallbacks_title", "Widget name fallbacks"),
        "\n".join(lines),
        parent=parent,
    )
...
class MainWindow(
    ...
    def __init__(self):
        ...
        self.title(
            tr("main_window.title_version", "CTkMaker v{version}").format(
                version=__version__,
            ),
        )
        ...
        # Two deques: preview output trims FIFO at 5000 entries; editor
        # logs are protected from preview floods at a separate 1000 cap
        # so a chatty preview never evicts a meaningful editor warning.
        # ``maxlen`` on ``deque`` gives O(1) auto-trim per append — no
        # explicit ``del buffer[:N]`` block needed anywhere downstream.
        self._preview_buffer: deque[tuple[str, str, str]] = deque(maxlen=5000)
        self._editor_buffer: deque[tuple[str, str, str]] = deque(maxlen=1000)
        ...
        # Level filter checkboxes — elide-based, persisted across
        # launches. Default everything visible except DEBUG (typical
        # Unity default). No source filters — the ``[E]`` prefix on
        # editor lines is enough of a visual indicator, and folding
        # an entire source behind an OR-elide tag turned out to be
        # confusing (silently hid every line).
        _filter_defaults = {
            "info": True, "warning": True, "error": True,
            "debug": False,
        }
        _saved_filters = load_settings().get("console_filters") or {}
        self._console_filter_vars: dict[str, tk.BooleanVar] = {}
        for _key, _default in _filter_defaults.items():
            v = tk.BooleanVar(
                value=bool(_saved_filters.get(_key, _default)),
            )
            v.trace_add(
                "write",
                lambda *_a, _k=_key: self._on_console_filter_changed(_k),
            )
            self._console_filter_vars[_key] = v
        # Live counters per severity, summed across editor + preview.
        # Updated by the drainer; read by ``ConsolePanel`` to render
        # toolbar badges.
        self._console_counts: dict[str, int] = {
            "info": 0, "warning": 0, "error": 0,
        }
        # Wire the editor-side logging handler + log_error forward to
        # the queue-routed entry point. Done as early as possible so
        # any module-level logger that fires during the rest of
        # ``__init__`` (Toolbar build, project load, etc.) makes it
        # into the console buffer.
        install_console_sink(self._push_editor_line)
        ...
        self._btn_tree = ctk.CTkButton(
            _pill_top,
            text=tr("main_window.tab_object_tree", "Object Tree"),
            command=_show_tree,
        ...
        self._btn_hist = ctk.CTkButton(
            _pill_top,
            text=tr("main_window.tab_history", "History"),
        ...
        self._btn_props = ctk.CTkButton(
            _pill_bot,
            text=tr("main_window.tab_properties", "Properties"),
            command=_show_properties,
            fg_color=_PILL_ACT, text_color=_ACT_FG,
            hover_color=_PILL_ACT, **_btn_kw,
        )
        self._btn_proj = ctk.CTkButton(
            _pill_bot,
            text=tr("main_window.tab_assets", "Assets"),
    ...
    def _refresh_title(self) -> None:
        from app import __version__
        base = tr("main_window.title_version", "CTkMaker v{version}").format(
            version=__version__,
        )
    ...
    def _confirm_discard_if_dirty(self) -> bool:
        ...
        reply = ask_yes_no_cancel(
            tr("main_window.unsaved_changes_title", "Unsaved changes"),
            tr(
                "main_window.save_before_continuing",
