...
def _ctk_kwargs(cls: type) -> dict[str, object]:
    ...
    for p in sig.parameters.values():
        if p.name in ("self", "master") or p.kind in (
            ...
            continue
        out[p.name] = (
            p.default if p.default is not inspect._empty
            else tr("inspector.required", "<required>")
        )
    return out
...
def _property_kind(descriptor, prop_name: str, default_value) -> str:
    ...
    for entry in getattr(descriptor, "property_schema", ()) or ():
        if entry.get("name") == prop_name:
            ...
            if raw:
                schema_type = tr(
                    f"inspector.schema_{raw}", schema_type,
                )
            break
    if not schema_type:
        # CTk-only kwarg: infer from default value's runtime type.
        if isinstance(default_value, bool):
            schema_type = tr("inspector.kind_bool", "bool")
        elif isinstance(default_value, int):
            schema_type = tr("inspector.kind_number", "number")
        elif isinstance(default_value, float):
            schema_type = tr("inspector.kind_number", "number")
        elif isinstance(default_value, str):
            schema_type = tr("inspector.kind_string", "string")
        elif callable(default_value):
            schema_type = tr("inspector.kind_callable", "callable")
    ...
    if prop_name in (getattr(descriptor, "multiline_list_keys", set()) or ()):
        ...
        schema_type = tr("inspector.kind_newline_string", "newline-string")
    if prop_name in (getattr(descriptor, "_NODE_ONLY_KEYS", set()) or ()):
        badges.append(tr("inspector.badge_node_only", "node-only"))
    if prop_name in (getattr(descriptor, "recreate_triggers", set()) or ()):
        badges.append(tr("inspector.badge_recreate", "recreate-on-change"))
...
class WidgetInspectorWindow(ManagedToplevel):
    ...
    def __init__(self, master):
        ...
        self.window_title = tr("inspector.title", "Inspect CTk Widget")
    ...
    def _build_chrome(self, parent, labels: list[str]) -> None:
        ...
        ctk.CTkLabel(
            bar, text=tr("inspector.widget_label", "Widget:"),
            text_color=ROW_FG,
            font=ui_font(11),
        ...
        legend = ctk.CTkLabel(
            ...
            text=(
                f"  {STATUS_EXPOSED} "
                f"{tr('inspector.legend_exposed', 'exposed')}     "
                f"{STATUS_RENAMED} "
                f"{tr('inspector.legend_renamed', 'renamed')}     "
                f"{STATUS_CTK_ONLY} "
                f"{tr('inspector.legend_ctk_only', 'CTk-only')}     "
                f"{STATUS_BUILDER_ONLY} "
                f"{tr('inspector.legend_builder', 'builder helper')}"
            ),
    ...
    def _build_table(self, parent) -> None:
        ...
        self._tree.heading("name", text=tr("inspector.col_property", "Property"))
        self._tree.heading("kind", text=tr("inspector.col_kind", "Kind"))
        self._tree.heading("ctk_default", text=tr("inspector.col_ctk_default", "CTk default"))
        self._tree.heading("builder_default", text=tr("inspector.col_builder_default", "Builder default"))
    ...
    def _populate(self, descriptor, preset: dict) -> None:
        ...
        for name in ctk_keys:
            ...
            if valid_targets:
                ...
                for t in valid_targets:
                    if t.startswith("("):
                        kind_parts.append(tr("inspector.kind_behavior", "behavior"))
        ...
        self._status_lbl.configure(
            text=(
                f"{tr('inspector.footer_ctk_class', 'CTk class:')} {ctk_label}    "
                f"{STATUS_EXPOSED} {good} "
                f"{tr('inspector.legend_exposed', 'exposed')}    "
                f"{STATUS_RENAMED} {renamed} "
                f"{tr('inspector.legend_renamed', 'renamed')}    "
                f"{STATUS_CTK_ONLY} {warn} "
                f"{tr('inspector.legend_ctk_only', 'CTk-only')}    "
                f"{STATUS_BUILDER_ONLY} {helper} "
                f"{tr('inspector.legend_builder', 'builder helper')}"
            ),
        )
