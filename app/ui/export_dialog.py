...
class ExportDialog(ManagedToplevel):
    ...
    def __init__(self, parent, project, preselected_doc_id=None):
        self.window_title = tr("export.window_title", "Export")
    ...
    def _build_scope_list(self) -> list[tuple[str, str]]:
        ...
        if is_multi_page:
            ...
            if n > 1:
                out.append((
                    tr("export.scope_all_pages", "All pages ({n} pages)").format(n=n),
                    ALL_PAGES_VALUE,
                ))
            ...
            for entry in pages:
                if not isinstance(entry, dict):
                    continue
                name = (entry.get("name") or "").strip() or tr("export.untitled", "Untitled")
                out.append((
                    tr("export.scope_page", "Page: {name}").format(name=name),
            ...
            if len(docs) > 1:
                for doc in docs:
                    dname = (doc.name or "Untitled").strip() or tr("export.untitled", "Untitled")
                    out.append((tr("export.scope_document", "Document: {name}").format(name=dname), doc.id))
            return out

        if len(docs) > 1:
            n = len(docs) - 1
            label = tr(
                "export.scope_all_forms",
                "All forms (Main + {n} Dialog{s})",
            ).format(n=n, s="" if n == 1 else "s")
        else:
            label = tr("export.scope_whole_project", "Whole project")
        out.append((label, ALL_FORMS_VALUE))
        for doc in docs:
            name = (doc.name or "").strip() or tr("export.untitled", "Untitled")
    ...
    def _build(self, container) -> None:
        ...
        ctk.CTkLabel(
            self._panel, text=tr("export.export_project", "Export Project"),
            font=ui_font(11, "bold"),
            text_color=SUBTITLE_FG, anchor="w",
        ).pack(fill="x", padx=14, pady=(10, 10))

        self._add_row(tr("export.name", "Name"), self._build_name_entry)
        self._add_row(tr("export.save_to", "Save to"), self._build_save_row)
        self._build_preview_label()
        self._add_separator()
        self._add_row(tr("export.scope", "Scope"), self._build_scope_row)
        self._add_separator()
        self._add_row(tr("export.format", "Format"), self._build_format_checkbox)
        self._add_separator()
        self._add_row(tr("export.comments", "Comments"), self._build_descriptions_checkbox)
        self._add_separator()
        self._add_row(tr("export.after", "After"), self._build_after_checkbox)
        self._build_footer()
    ...
    def _build_save_row(self, row) -> None:
        ...
        self._attach_tooltip(
            browse, "browse", tr("export.tooltip_browse", "Browse for the export folder."),
        )
    ...
    def _build_scope_row(self, row) -> None:
        ...
        self._attach_tooltip(
            dropdown, "scope",
            tr("export.tooltip_scope",
               "What to export — every page, one page, or a single "
               "form of the active page."),
        )
        n_forms = len(self.project.documents)
        if n_forms == 1:
            info = tr("export.forms_in_project_one", "{n} form in project").format(n=n_forms)
        else:
            info = tr("export.forms_in_project_many", "{n} forms in project").format(n=n_forms)
        stk.Label(
    ...
    def _build_format_checkbox(self, row) -> None:
        ...
        zip_cb = ctk.CTkCheckBox(
            zip_row, text=tr("export.as_zip", "Export as ZIP archive"),
        ...
        if self.project.folder_path:
            ...
            filter_cb = ctk.CTkCheckBox(
                filter_row, text=tr("export.only_used_assets", "Include only used assets"),
    ...
    def _build_descriptions_checkbox(self, row) -> None:
        ...
        desc_cb = ctk.CTkCheckBox(
            desc_row, text=tr("export.include_descriptions", "Include descriptions as comments"),
        ...
        self._attach_tooltip(
            ...
            tr("export.tooltip_descriptions",
    ...
    def _build_after_checkbox(self, row) -> None:
        ...
        self._open_editor_cb = ctk.CTkCheckBox(
            row, text=tr("export.open_in_editor", "Open in editor"),
        ...
        self._attach_tooltip(
            ...
            tr("export.tooltip_after_folder",
    ...
    def _build_footer(self) -> None:
        ...
        export_btn = ctk.CTkButton(
            footer, text=tr("export.export_btn", "Export"), width=160, height=32,
            corner_radius=4, command=self._on_export,
        )
        export_btn.pack(side="right")
        self._attach_tooltip(
            export_btn, "export",
            tr("export.tooltip_export",
               "Write the export to the path shown in the preview line."),
        )
        cancel_btn = ctk.CTkButton(
            footer, text=tr("export.cancel_btn", "Cancel"), width=90, height=32,
            corner_radius=4,
            fg_color="#3c3c3c", hover_color="#4a4a4a",
        ...
        self._attach_tooltip(
            cancel_btn, "cancel", tr("export.tooltip_cancel", "Close without exporting."),
        )
    ...
    def _on_browse_folder(self) -> None:
        ...
        chosen = filedialog.askdirectory(
            parent=self, title=tr("export.export_folder_title", "Export folder"),
    ...
    def _on_export(self) -> None:
        ...
        if target is None:
            show_warning(
                tr("export.missing_fields_title", "Missing fields"),
                tr("export.missing_fields_msg",
        ...
        try:
            ...
            show_error(
                tr("export.export_failed_title", "Export failed"),
                tr("export.create_folder_failed_msg",
                   "Could not create folder:\n{path}\n\n{error}").format(
                    path=target.parent, error=exc,
                ),
                parent=self,
            )
            return
        ...
        try:
            ...
            show_error(
                tr("export.export_failed_title", "Export failed"),
                tr("export.write_file_failed_msg",
        ...
        if not (do_editor or do_preview or do_folder):
            show_info(
                tr("export.export_info_title", "Export"),
                tr("export.saved_to_msg", "Saved to:\n{path}").format(path=target),
                parent=self,
            )
        self.destroy()
    ...
    def _confirm_overwrite(self, target: Path) -> bool:
        ...
        if self._as_zip_var.get():
            ...
            ).format(name=target.name, path=target.parent)
        else:
            bundle = target.parent
            if not bundle.exists():
                return True
            question = tr(
                "export.folder_exists_msg",
                'A folder named "{name}" already exists in:\n{path}\n\n'
                "Export into it anyway? Files with the same names will "
                "be overwritten.",
            ).format(name=bundle.name, path=bundle.parent)
        return ask_yes_no(
            tr("export.export", "Export"), question, parent=self, danger=True,
        )
    ...
    def _export_single_page(
        ...
        if clone is None:
            show_error(
                tr("export.export_failed_title", "Export failed"),
                tr("export.load_page_failed_msg",
    ...
    def _run_exported_preview(self, path: Path) -> None:
        ...
        try:
            ...
            show_error(
                tr("export.preview_failed_title", "Preview failed"),
                tr("export.preview_failed_msg",
