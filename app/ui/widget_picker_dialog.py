...
class WidgetPickerDialog(ManagedToplevel):
    ...
    def __init__(
        ...
        self.window_title = tr(
            "widget_picker.window_title", "Pick widget for {field_name}"
    ...
    def _build_ui(self, field_name: str, expected_type: str) -> ctk.CTkFrame:
        ...
        ctk.CTkLabel(
            header,
            text=tr(
                "widget_picker.header_title",
                "Pick widget for «{field_name}»",
            ).format(field_name=field_name),
        ...
        if self._current_widget_id:
            ctk.CTkButton(
                footer, text=tr("widget_picker.clear", "Clear"), width=80,
                fg_color=_BTN_BG, hover_color=_BTN_HOVER,
                text_color=_HEADING_FG,
                command=self._clear,
            ).pack(side="left")
        ctk.CTkButton(
            footer, text=tr("widget_picker.cancel", "Cancel"), width=80,
            fg_color=_BTN_BG, hover_color=_BTN_HOVER,
            text_color=_HEADING_FG,
            command=self._cancel,
        ).pack(side="right", padx=(8, 0))
        self._pick_btn = ctk.CTkButton(
            footer, text=tr("widget_picker.pick", "Pick"), width=80,
    ...
    def _insert_node(self, node, parent_iid: str) -> None:
        ...
        suffix = "" if compatible else tr(
            "widget_picker.wrong_type", "  — wrong type"
        )
