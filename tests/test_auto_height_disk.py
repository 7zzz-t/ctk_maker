...
- ``to_dict``:  height 0  →  height=200 + marker
- ``from_dict``: marker present  →  height restored to 0 (marker dropped)
...
def test_auto_frame_disk_form_carries_snapshot_and_marker():
    node = _frame(0)
    disk = node.to_dict()
    assert disk["properties"]["height"] == 200
    assert disk["properties"]["_ctkmaker_auto_height"] is True
...
def test_roundtrip_restores_auto_height_in_memory():
    ...
    assert "_ctkmaker_auto_height" not in restored.properties
...
def test_nested_auto_frame_is_mapped_recursively():
    ...
    child_disk = disk["children"][0]["properties"]
    assert child_disk["height"] == 200
